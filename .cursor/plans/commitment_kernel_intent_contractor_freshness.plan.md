...
todos:
  ...
  - id: freshness-phase-5-acceptance
    content: "Phase 5 — Acceptance + log-line evidence + live-verify. NEW `src/platform/commitment/__tests__/intent-contractor-freshness.acceptance.test.ts` (~6 cases): (1) High-N memory cohort — operator with 120 recalled entries; top-K post-decay matches newest-first within 1-day window. (2) Mixed-age `<active_tasks>` — nudged-yesterday surfaces above created-week-ago. (3) Default config produces `<freshness_hints>` with 7-day half-life. (4) Reverse — pre-Phase-4 byte-identical when `freshnessConfig=undefined` AND every entry has no timestamp. (5) Reverse — entries with FUTURE timestamps (clock-skew) yield `recencyDecay=1.0` capped; never above 1.0. (6) Identity-isolation invariant unchanged — freshness reorder NEVER promotes other identity entries. **Live-verify (REQUIRED — invariant #15 + handoff)**: gateway restart + 3 live operator turns where prior memory cohort exceeds 10 entries. live-verifier asserts: `[intent-contractor] freshness.applied entries=<N> halfLifeMs=<N> windowMs=<N>`; `[intent-contractor] memory.block injected entries=<N> recencyDecayMin=<f> recencyDecayMax=<f>`; `[intent-contractor] active_tasks.block injected tasks=<N> oldestAgeMs=<N>`; Telegram reply remains coherent. Reverse: zero-entry recall produces zero `<freshness_hints>` injection. Slice CLOSED."
    status: pending
isProject: false
---
