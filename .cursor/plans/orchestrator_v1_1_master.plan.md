...
| P1.4     | Stage A ✅ (ledger v0, heuristic, injection в classifier, `[intent-ledger] peek/injected` в логе). Stage B ✅ (evidence reconciler + `violated_promise` + hard-replan budget, `[evidence] promises/receipts/violations/action`, live 13/13 включая `13-confirmation-yes-exec` с `toolCall name=exec`). Stage C (Progress Bus) и Stage D (Ack-then-defer) pending. | P1        |
