...
**P1.4 Stage C ✅ (2026-04-21, live `15/15` включая `14-progress-bus`; `[progress] gateway bridge attached`, WS `progress.frame`, Telegram adapter ready)**,
P1.2/P1.1 и P1.4 D pending
...
| P1.4     | Stage A ✅ (ledger v0, heuristic, injection в classifier, `[intent-ledger] peek/injected` в логе). Stage B ✅ (evidence reconciler + `violated_promise` + hard-replan budget, `[evidence] promises/receipts/violations/action`, live 13/13 включая `13-confirmation-yes-exec` с `toolCall name=exec`). **Stage C ✅ (Progress Bus: `ProgressFrame { sessionId, channelId, turnId, seq, phase, detail, meta, ts }`, эмитеры в classifier/planner/agent-runner, gateway bridge `progress.frame` broadcast, Telegram edit-adapter готов, live 15/15 включая `14-progress-bus`).** Stage D (Ack-then-defer) pending. | P1        |
