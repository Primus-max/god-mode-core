...
входит в P1.6».
- **P1.6 (post-smoke follow-ups, 2026-04-22): DONE** (три этапа закрыты,
 unit + lint зелёные; live smoke `20-intent-idempotency` добавлен в скрипт,
 общий `pnpm live:routing:smoke`-прогон выполняется при следующем gateway-тесте):
  ...
  - **P1.6.3 ✅** (2026-04-22) — детерминистический `intent-fingerprint`
  (`session/intent-fingerprint.ts`) + read-API `lookupRecentReceipt` в
  `intent-ledger`; preflight short-circuit в `auto-reply/reply/agent-runner-execution.ts`
  возвращает «уже сделано: <url/PID/path>» для повторных намерений в окне 60s
  (env `OPENCLAW_INTENT_IDEMPOTENCY_WINDOW_MS`, `0` — kill-switch).
  Channel-agnostic, без LLM-эвристик.
...
| ~~P1.6.3~~   | ~~Нет идемпотентности: «подними сервер» ×3 = три vite-процесса. Нужен `intent-fingerprint` + lookup в receipts ledger перед `runRecipe()`, channel-agnostic, чисто детерминистический (не LLM).~~ Закрыт 2026-04-22: добавлен deterministic fingerprint + short-circuit lookup свежего successful receipt в ledger до embedded/runtime запуска; kill-switch `OPENCLAW_INTENT_IDEMPOTENCY_WINDOW_MS=0`, live smoke `20-intent-idempotency`. | P1.6 (done) |
...
.cursor/plans/orchestrator_v1_1_p1_6_post_smoke_followups.plan.md ← provider-scoped requiredEnv + exec-evidence relax + intent-fingerprint
