...
| --- | --- | --- | --- | --- | --- |
| 2026-04-29 | PR-G — subagent-await cross-turn lifecycle gate | `fix/orchestrator-subagent-await` | `d0b3c3fc33` | [#111](https://github.com/Primus-max/god-mode-core/pull/111) | Closes roadmap step 1. Cross-turn gate per `(sessionId, turnId)`; holding-payload + final consolidate-reply emitted with idempotency keys; subagent terminal timeout fallback wired (`acceptanceReason='subagent_terminal_timeout'`). Forward-compat constraints from §4 satisfied. |
| 2026-05-01 | Stabilize `feat/commitment-preflight-slice` (cleanup) | `feat/commitment-preflight-slice` | TBD on commit | — | Reverted invariant-violating phrase-rule (`shouldDowngradePlainChatPersistentWorker` in `decision/input.ts` + downstream `isInternalRespondOnlyTurn` short-circuit in `agent-command.ts`) introduced by junior model. Fixed `pnpm tsgo` regression on Vladimir's preflight slice (preflight.ts uses local narrow types; `agent-via-gateway.ts:264` decision shape). Deleted shadow `runner.ts` / `execution-loop.ts` / `execution-state.ts` / `execution-runtime.ts` / `execution-seed.ts` / `command-execution-runtime.ts` / `trace.ts` stack — it duplicated kernel pipeline without using `MonitoredRuntime` / `Affordance` / `DonePredicate`. Branch now contains only: (a) Vladimir's 3 preflight commits with tsgo-green types; (b) orthogonal junior fixes preserved (subagent dedupe, agents-list config injection, cron pagination, fast-core-tools mocks, agent-runner routingSnapshot). 173 scoped tests green. **Next: roadmap step 2 = PR-A.2 — block-streaming buffering при наличии tool_call** (PR-A.2 уже стартовал на dev: 2 коммита 3df3138fcc + ed8a9d137f; нужно завершить + tests + sub-plan handoff). После PR-A.2 → PR-H. |
