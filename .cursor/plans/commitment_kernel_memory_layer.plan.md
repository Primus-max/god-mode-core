...
### 2026-05-05 — Phase 4 implemented (Path B chosen — in-house extractor)

- Phase 4 `e-phase-4-mem0-wrapper-or-fallback` shipped via Path B (in-house `LlmExtractorMemoryStore`).
- Decision-gate audit: the `mem0ai` Node SDK is alpha-quality (sub-plan §2.2 noted "JS SDK is ALPHA-quality but functional for the wrapper-pattern use case"). The agent worktree sandbox blocked direct package install + registry probes (`pnpm install`, `npm view`, `node` script execution all denied), so an empirical Node-API audit was not possible from the slice-implementer environment. Per sub-plan §2.2 ("If the API is unstable enough that wrapping it costs more than implementing the LLM-extraction layer ourselves, surface as a §6 amendment proposal and proceed with the pure-sqlite-vec path") and §6 ("If denied or audit fails, ship in-house `LlmExtractor` and surface §6 D1 as needing amendment"), Path B was selected.
- **§6 D1 amendment proposal** (informational; non-blocking): the master roadmap §6 D1 closed decision named mem0 as the LLM-extraction layer. This phase ships an in-house, single-LLM-call extractor instead. The public `MemoryStore` interface from Phase 1 is unchanged → swap path is a one-line constructor change if a future sub-plan amendment re-introduces mem0. Recommend the next roadmap revision either (a) re-confirm Path A is required and budget a follow-up slice to wrap `mem0ai` once its Node SDK ships a stable 1.x, or (b) ratify Path B and update §6 D1 to read "LlmExtractorMemoryStore (in-house) on top of sqlite-vec". This PR does NOT presume the outcome.
- Files added (all new) on branch `feat/v1-slice-e-memory-extraction-layer`:
  - `src/platform/memory/llm-extractor-prompt.ts` — frozen `LLM_EXTRACTOR_PROMPT` text + `LlmExtractorDecisionSchema` (Zod discriminated union on `keep`) + `parseLlmExtractorDecision` helper (handles ```json``` code-fence wrappers).
  - `src/platform/memory/llm-extractor-prompt.test.ts` — schema validation tests (well-formed + 5 negative paths) + parser code-fence stripping.
  - `src/platform/memory/llm-extractor-store.ts` — `LlmExtractorMemoryStore` impl + `LlmExtractor` / `LlmExtractorMemoryStoreLogger` / `LlmExtractorMemoryStoreOptions` injection seams; `EXTRACTOR_TAGS_METADATA_KEY = "extractor_tags"` for downstream consumers.
  - `src/platform/memory/llm-extractor-store.test.ts` — pluggability suite (mirrors Phase 2 in-memory acceptance shape) + extraction-on round-trip + extraction-off (`keep:false` → no row) + extractor-crash → passthrough + warning + identity isolation through wrapper + SqliteVec-delegate pluggability.
  - `src/platform/memory/index.ts` — barrel exports the new types + class.
- LLM provider for extraction: NOT bound by this module. The `LlmExtractor` injection seam keeps provider choice at the call site. Production wiring (slice E Phase 5 commitment-runtime hook) will read the existing memory-config schema and pass an OpenAI / Gemini / Mistral / Voyage / Ollama-backed adapter; this PR does NOT add a parallel provider config.
- `package.json` was NOT modified. Path B requires no third-party runtime dep — the extractor is a single LLM call with a Zod-validated structured output.
- Hard invariants honoured:
  - **#5/#6** — extractor reads only the structured `content: string` field of `SemanticMemoryWrite`; never `RawUserTurn`/`UserPrompt`.
  - **#8** — no import from `src/platform/decision/` or `src/platform/commitment/`.
  - **#11** — `MemoryEntryId` brand preserved; kept entries use the delegate's mint, dropped entries use a `mem:dropped-<uuid>` sentinel that still passes the brand validator.
  - **#15** — extraction is observability, not gating: extractor crash → passthrough write + `extractor_error` warning, the underlying delegate stays callable.
  - **Pluggability** — `Mem0MemoryStore`, `SqliteVecMemoryStore`, `InMemoryMemoryStore`, `LlmExtractorMemoryStore` all conform to the Phase-1 `MemoryStore` interface. The wrapped delegate is itself a `MemoryStore`, so the extraction layer composes cleanly with Phase 2's in-memory store and Phase 3's sqlite-vec store.
