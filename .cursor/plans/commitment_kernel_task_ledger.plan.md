...
### 2026-05-05 — Phase 1 audit completed

- Branch: `audit/v1-slice-f-task-ledger-phase-1`. Predecessor: `c644a9b794` (PR #180 — sub-plan kickoff). Merge SHA: TBD (filled after maintainer admin-merge).
- Deliverable: `extensions/AUDIT-task-ledger.md` — line-anchored audit covering §2.1 task surfaces, §2.2 effect-family + episodic slot status, §2.3 `<active_tasks>` recall site, §2.4 `TargetRef` extension recommendation, §2.5 identity surface, §2.6 cross-reference confirmation, §5 wiring site, §8 pre-existing emitters scan.
- Maintainer signoff: blanket v1 commitment-kernel slice signoff granted by Vladimir 2026-05-05 covers this read-only phase.
- **§2.4 CRITICAL recommendation**: ship slice F WITHOUT extending `TargetRef`. The orchestrator-level UX («отмени PDF» / «как там?») is fully serveable via the Phase 6 `<active_tasks>` block (id+label+status injection) PLUS a free-form `constraints.taskRef` field that the LLM emits — `SemanticIntent.constraints` is `ReadonlyRecord<string, unknown>` (open record, `intent-contractor-impl.ts:121` Zod uses `z.record(z.string(), z.unknown())` without `.strict()`), so the field passes through today without ANY frozen-layer change. Frozen-layer touch (`semantic-intent.ts` discriminated-union widening + matching Zod schema in `intent-contractor-impl.ts:100` + LLM prompt examples + eval golden set) is therefore deferred and not required by slice F.
- **§2.2 / §2.6 audit correction**: the sub-plan §2.2 line stated «slice E typed-but-inert task.* slots». **`EpisodicMemoryEvent` discriminated union (`src/platform/memory/episodic-memory-event.ts:20–24`) does NOT contain a `"task"` member today** — only `persistent_session`, `subagent`, `reminder`, `artifact`. Slice F Phase 2 MUST extend `EpisodicEffectFamily` with `"task"` plus the matching payload type (`TaskCreatedPayload` etc.) and update `EpisodicMemoryEventSchema` (lines 190–216) AND the switch in `memory-write-on-satisfied.ts:195–245` (the `assertNeverEpisodicEventInput` exhaustiveness guard will fire on the unhandled new variant — that is the desired behaviour and forces lock-step extension). Phase 2 scope is unchanged otherwise; the discriminated-union extension is additive (new members, no breaking change for existing readers once they update their switch).
- **§8 pre-existing emitters scan**: ZERO `task.*` emit sites discovered across `src/`, `extensions/`, and plugin-SDK (greps for `task.created|task.completed|task.cancelled|task.failed`, `effectFamily.*task`, `task.\\*`). No allowlist filter needed in the Phase 5 hook — slice F introduces both the surface and its sole emit site.
- **§5 wiring site recommendation**: extend `src/platform/decision/memory-wiring.ts` (Strategy A) so `onAttestation` fans out to BOTH the slice E memory hook AND the slice F task hook. The single-callback discipline at `run-turn-decision.ts:294` is preserved; the wiring helper is in the decision layer (NOT the frozen commitment layer) and was already touched by slice E PR-#161 — sanctioned.
- **§2.5 identity surface**: confirmed `resolveIdentityFromSessionKey` is the SOLE entrypoint; slice F uses as-is. Anonymous + wrapped scopes (`subagent`/`cron`/`acp`) return `undefined` — cron-fired tasks need their own context-injection path (slice J concern).
- **`pnpm exec tsgo --noEmit`** clean (exit 0). Read-only deliverable; no source changes.
- **Phase 2 unblocked.** Awaiting per-phase signoff before commit.
