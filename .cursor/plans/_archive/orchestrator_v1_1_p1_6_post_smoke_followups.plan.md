...
## Этап P1.6.3 — Intent fingerprint + idempotency window [⚠️ отозван] (2026-04-22)

> **2026-04-22 поздний вечер.** Этот этап **отозван по архитектуре** после
> второго ручного Telegram-смоука. Реализация в коде осталась (см. файлы
> ниже), но помечена как технический долг. Корректное решение —
> **P1.7-D** (см. `orchestrator_v1_1_p1_7.plan.md`): расширить
> `evidence-sufficiency.evaluate` явным `priorEvidence` (первый probe —
> ledger), без time-window и без runner-preflight. Запись об отзыве — в
> мастер-плане §0/§2/§8 от 2026-04-22 (поздний вечер). Ниже сохранён
> исходный текст этапа как историческая запись.
