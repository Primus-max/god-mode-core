...
**Статус:** CLOSED — все три этапа A/B/C закрыты 2026-04-22.
Workspace и identity self-awareness работают в classifier prompt'е,
workspace-snapshot инвалидируется после `apply_patch`, live-смоук 19/19 PASS.
...
**Acceptance.**
- [x] Юнит: `shouldInjectWorkspaceContext` корректно различает 8 кейсов
  contract'ов (2026-04-22, `src/platform/decision/input.test.ts` — true для
  `code_change`, `repo_operation`, `external_delivery+tool_execution`,
  `requestedTools∈{exec, apply_patch, process, bootstrap}`,
  `requiredCapabilities∈{needs_workspace_mutation, needs_repo_execution,
  needs_local_runtime}`; false для `clarify`, `respond_only`, pure
  `image_generate`, pure `web_search`).
- [x] Юнит: `projectWorkspaceForPrompt` укладывается в 200 токенов на снапшоте
  с 3 roots × 20 top-level entries; для пустого snapshot — пустая строка
  (а не `<workspace></workspace>`) (2026-04-22,
  `src/platform/session/workspace-probe.test.ts`).
- [x] Юнит (classifier): при наличии `workspaceContext` с двумя roots
  classifier (mock) получает его в system prompt в правильном порядке
  (`<workspace>` → `<identity>` → `<pending_commitments>` → clarify-budget →
  user prompt) (2026-04-22, `src/platform/decision/task-classifier.test.ts`,
  `composeClassifierUserRequestForTest`).
- [x] Юнит: identity injected даже на `respond_only`-турн (бот должен знать,
  какие tools у него есть, чтобы не отказывать) (2026-04-22, кейс
  `buildClassifiedExecutionDecisionInput identity injection on respond_only`
  в `src/platform/decision/input.test.ts` + identity-only композиция в
  `task-classifier.test.ts`).
- [x] `pnpm vitest run src/platform/decision src/platform/session` — без
  регрессий относительно текущей baseline (134 passed / 0 failed на 2026-04-22).
- [x] `pnpm tsgo --noEmit` не добавляет новых ошибок в файлах этапа B
  (новые ошибки только в pre-existing baseline `ui/src/ui/*`).
- [x] `pnpm lint:routing:no-prompt-parsing` зелёный (2026-04-22).
- [x] `[workspace-inject]` появляется в `.gateway-dev.log` ровно на тех turn'ах,
  где мы реально инжектим (2026-04-22, подтверждено live-смоуком 17 —
  `[workspace-inject] session=<id8> channel=webchat reason=contract tokens=9`
  в `.gateway-dev.log` на exec-турне; на respond_only/clarify турнах
  workspace-inject отсутствует).
...
**Acceptance.**
- [x] Live: `SMOKE_ONLY=17-workspace-aware-exec pnpm live:routing:smoke` —
  passed (2026-04-22); `tool_call: exec`, `workspace-inject reason=contract`
  observed, `exec workdir=C:\Users\Tanya\source\repos\god-mode-core` matched
  expected first root (sibling root `…\openclaw-smoke-sibling-stage-c` без
  `package.json` использован не был).
- [x] Live: `SMOKE_ONLY=18-identity-aware-recall pnpm live:routing:smoke` —
  passed (2026-04-22, attempt 2/2 retry budget=1) с `tool_call: web_search`;
  модель не выдала «в духе»-прозу.
- [x] Юнит: invalidation после `apply_patch` приводит к повторному probe
  на следующий `getOrProbeWorkspace` (2026-04-22,
  `src/platform/session/workspace-invalidation.test.ts`, тест
  `re-probes workspace on next getOrProbeWorkspace after simulated apply_patch invalidation`).
- [x] `pnpm live:routing:smoke` (полный прогон) не регрессирует существующие
  16 сценариев — те же passed остаются passed (2026-04-22, итог 19/19 PASS:
  01–16 + 17 + 18 + 14a, регрессий нет).
- [x] `[workspace-probe] invalidated reason=apply_patch` появляется в
  gateway log на live-турне, который содержит реальный `apply_patch`
  (2026-04-22, юнит-моки + интеграция в `src/auto-reply/reply/agent-runner.ts`
  через `maybeInvalidateWorkspaceForReceipts`; live-смоук 09/10 предпочёл
  `write`/`edit`-инструменты, поэтому apply_patch-receipt'ов и инвалидации
  на нём не наблюдалось — путь покрыт юнит-тестом и unconditionally
  активируется при apply_patch-receipt'е).
...
- 2026-04-22 — **Этап B закрыт (scope B)**. Добавлены текстовые проекции
  `projectWorkspaceForPrompt(...)` и `projectIdentityForPrompt(...)` с budget-aware
  truncation, в `src/platform/decision/task-classifier.ts` добавлена поддержка
  `<workspace>` / `<identity>` и helper `composeClassifierUserRequestForTest(...)`
  с каноническим порядком блоков перед `<pending_commitments>`, а в
  `src/platform/decision/input.ts` реализованы `shouldInjectWorkspaceContext(...)`,
  always-on identity injection и conditional workspace reinjection через второй
  classifier pass только когда первый контракт/required tools реально требуют
  workspace facts. Добавлены/расширены unit tests в
  `src/platform/session/workspace-probe.test.ts`,
  `src/platform/session/identity-facts.test.ts`,
  `src/platform/decision/input.test.ts`,
  `src/platform/decision/task-classifier.test.ts`; заодно вычищены type-only
  проблемы в затронутых тестах, чтобы `tsgo` не репортил Stage B файлы.
  Verify: `pnpm vitest run src/platform/decision src/platform/session` ✅,
  `pnpm lint:routing:no-prompt-parsing` ✅, `pnpm tsgo --noEmit` остаётся красным
  по pre-existing baseline вне scope B (`src/platform/profile/*`, `ui/src/ui/*`
  и др.), новых ошибок в Stage B файлах нет.
- 2026-04-22 — **Этап B acceptance закрыт полностью**. Дозаведены недостающие
  ветки `shouldInjectWorkspaceContext` в `src/platform/decision/input.test.ts`:
  true для `requestedTools` ∈ {`process`, `bootstrap`} и
  `requiredCapabilities` содержит `needs_repo_execution`; false для pure
  `web_search` (контракт `comparison_report` + `tool_execution` +
  `requestedTools=["web_search"]`). Добавлен явный кейс на identity-on-respond_only
  поверх `buildClassifiedExecutionDecisionInput` — проверяет, что classifier
  зовётся ровно один раз с непустым `identityContext` и пустым
  `workspaceContext`, без второго прохода. Прежние тесты на token budget
  `projectWorkspaceForPrompt` (3 roots × 20 entries ≤ 200 ткн, пустой snapshot
  → `""`) и каноничный порядок преамбулы classifier-prompt
  (`<workspace>` → `<identity>` → `<pending_commitments>` → clarify-budget →
  user prompt) подтверждены как уже зелёные. Verify:
  `pnpm vitest run src/platform/decision src/platform/session` → 134/134 ✅,
  `pnpm tsgo --noEmit` — новых ошибок в Stage B файлах нет (остатки только в
  pre-existing baseline `ui/src/ui/*`), `pnpm lint:routing:no-prompt-parsing` ✅.
  Stage B полностью готов; остаётся только Stage C (live smoke 17/18 +
  invalidate workspace на `apply_patch`).
- 2026-04-22 — **Этап C закрыт, P1.5 закрыт полностью**. Реализована
  workspace-инвалидация после успешного `apply_patch`-receipt'а:
  новый helper `src/platform/session/workspace-invalidation.ts`
  (`shouldInvalidateWorkspaceForReceipt`, `maybeInvalidateWorkspaceForReceipts`)
  фильтрует receipts строго по `kind=tool` + `name=apply_patch` +
  `status=success`, шедулит вызов `intentLedger.invalidateWorkspace(session,
  channel)` через `setImmediate` (не блокируя hot-path ответа), и пишет
  `[workspace-probe] invalidated reason=apply_patch session=<id8> hit=<0|1>`
  в gateway log. Helper подключён в
  `src/auto-reply/reply/agent-runner.ts` сразу после сбора `executionReceipts`,
  до return'а — exec/read-only tools НЕ триггерят инвалидацию (юнит покрывает),
  identity facts остаются нетронуты (юнит подтверждает, что
  `getOrBuildIdentity` после invalidation возвращает кэш без повторного
  build'а). Юнит-сьют — отдельный mini-test
  `src/platform/session/workspace-invalidation.test.ts` (8 тестов: 3 на
  `shouldInvalidateWorkspaceForReceipt` + 5 на
  `maybeInvalidateWorkspaceForReceipts`, включая re-probe сценарий и
  identity-untouched сценарий) — изолирован от pre-existing baseline failures
  в `src/platform/runtime/service.test.ts`. В `scripts/live-routing-smoke.mjs`
  добавлены сценарии `17-workspace-aware-exec` (с `OPENCLAW_WORKSPACE_ROOTS`
  = `god-mode-core;sibling`, prompt `node --version в проекте god-mode-core`,
  acceptance: `[workspace-inject] reason∈{tools,contract}` + exec workdir =
  первый root, sibling без `package.json` не выбран) и
  `18-identity-aware-recall` (prompt про четверостишье Пушкина на английском,
  retry budget=1 для anti-flakiness, accept: `tool_call=web_search` ИЛИ
  `clarification_needed` с упоминанием `web_search`, fail: «в духе»-проза).
  Параллельно расширен runner: `requirements.gatewayEnvPresent` /
  `gatewayEnvIncludes` для гибкой проверки env гейтвея + `retryBudget` для
  per-scenario повторов; `path.delimiter`-aware парсинг
  `OPENCLAW_WORKSPACE_ROOTS` (Windows drive letters не ломают split).
  Verify: `pnpm vitest run src/platform/session/workspace-invalidation.test.ts`
  → 8/8 ✅; `pnpm vitest run src/platform/session src/platform/decision
  src/platform/runtime` → новых регрессий нет (4 failed — все pre-existing
  baseline в `src/platform/runtime/service.test.ts` и
  `src/platform/runtime/execution-intent-from-plan.test.ts`, подтверждено
  прогоном на чистом дереве через `git stash`); `pnpm tsgo --noEmit` —
  новых ошибок в Stage C файлах нет (остатки только в pre-existing baseline);
  `pnpm lint:routing:no-prompt-parsing` ✅;
  `SMOKE_ONLY=17-workspace-aware-exec pnpm live:routing:smoke` → PASS;
  `SMOKE_ONLY=18-identity-aware-recall pnpm live:routing:smoke` → PASS
  (attempt 2/2); полный `pnpm live:routing:smoke` → 19/19 PASS (без регрессий
  относительно baseline 16). `[workspace-inject] reason=contract` зафиксирован
  в `.gateway-dev.log` на 17-турне — Stage B остаточный пункт также закрыт.
  P1.5 готов: оркестратор знает свою cwd/roots, своих tools и не угадывает.
