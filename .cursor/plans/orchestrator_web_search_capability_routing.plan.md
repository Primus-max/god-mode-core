...
| --- | --- | --- | --- | --- |
| 1 | **Bundle-as-contract enforcement at the LLM schema layer** | When the classifier mis-emits `bundles=[respond_only] requestedTools=[]` but the user actually needs fresh data (turn `355ae135` line 157 — `Provider finish_reason: error` after autonomous web_search hits DDG bot-detection), no narrow routing gate can catch it because neither signal reaches preflight. Hard fix: thread `toolBundles` from planner output into `attempt.ts:1915` and filter the tool schema with a curated `BUNDLE_TO_TOOL_ALLOWLIST` (e.g. `respond_only` excludes `web_search`). | (a) New parameter `toolBundles?: ResolutionToolBundle[]` on `createOpenClawCodingTools`. (b) `BUNDLE_TO_TOOL_ALLOWLIST` curated map. (c) Wire the bundle from `attempt.ts` params (which itself needs the field threaded from planner output via the runner-execution glue). (d) Tests for every bundle × tool intersection. | **RESOLVED 2026-05-07** by `commitment_kernel_bundle_as_contract.plan.md` (SLICE COMPLETE; 8 PRs #286–#292 + Phase 7 closure). Implementation took the structurally cleaner adapter-layer route — a NEW `src/agents/bundle-schema-filter.ts` (pure helper) + `applyBundleSchemaFilterAtAttempt` adapter at `attempt.ts:2074-2087` reading `params.platformExecutionContext.resolutionContract.toolBundles` AFTER `applyModelProviderToolPolicy` (no `createOpenClawCodingTools` signature change required). Closed `DEFAULT_BUNDLE_ALLOWED_TOOLS` map covers all 9 enum values; symmetric reverse-defense fires for non-native-search models when bundle does not carry `public_web_lookup`; `[bundle-filter]` telemetry; empty `toolBundles` → byte-identical pass-through via `missingBundlePolicy: 'allow_all'` regression-guard. 5 frozen contracts BYTE-IDENTICAL (sha256 `75f926ef25f057f006ae9f48cd0eb624c392feea3206f6990e013df2cf23d25e`). |
