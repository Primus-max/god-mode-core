...
**Status (2026-05-06)**: All 4 bugs are tracked here as orthogonal to v1 commitment-kernel slices. Until they land, the Telegram end-to-end demo cannot pass live verification, even though all v1 slice acceptance fixtures are green and frozen contracts byte-identical.

**Master plan completion status (2026-05-06)**: v1 commitment-kernel slices CLOSED (E / F / I / Cutover-3 / PolicyGate Full / Search-Composer pipeline). cutover-4 (`repo_operation.completed`) now unblocked but not done. NEW-A/B/C/D + cutover-4 + Slice K (reminder) + Slice H P5/P6 + Bug A.2 (streaming buffering) + Bug F (persistent worker push) + Concurrent broker (PR-MT) + bundle-as-contract enforcement + IntentContractor freshness/recency = open frontier per §8 + §16. Master plan is **NOT** yet fully complete.
