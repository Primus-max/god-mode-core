...
| 2026-05-06 | **Live-test post-slice-F surfaced 4 production bugs** — orchestrator boots, slices D/E/F/I architecturally green, but Telegram-side execution shows: (1) `closure-outcome-dispatcher` returns `remediation: "bootstrap"` even when `Capability '<plugin>' was installed and verified` (false-positive at `src/auto-reply/reply/closure-outcome-dispatcher.ts:863` — comment acknowledges «no capabilities are pending bootstrap»; user-visible: «Your task is paused while a capability install is pending»); (2) `image_generate` tool wrapper does not pass user-attached image as reference into Hydra `/images/edits` — model generates from text prompt only, not from sketch; (3) PDF subagent timeout hard-coded `timeoutMs=2000` (`sessions_yield abort settle timed out` in log) — too short for actual rendering; (4) per-channel `streaming: "off"` in config does not block partial chunks emitted via `pi-embedded-subscribe.ts:385` `stripBlockTags` — chain-of-thought leaks to TG. **NONE of the 4 are slice E/F/I regressions** — they were either pre-existing in upstream (`closure-outcome-dispatcher`, streaming chunker) OR architectural gaps (`image_generate` reference passthrough, PDF subagent budget). **Master plan mapping** (per §27 / §49 / §8.5.1 / §16): bug #1 + #3 → **`commitment_kernel_policy_gate_full.plan.md`** (PolicyGate Stages 2-6: approvals/budgets/role-based/retry/escalation; obligatory before cutover-4); bug #2 → **`commitment_kernel_cutover3_artifacts.plan.md`** (cutover-3 for repo_operation/artifact effects via AffordanceRegistry — directly closes PDF/img2img/DOCX classification correctness); bug #4 → minor outbound follow-up (slice I Phase 7 OR standalone). **Also surfaced**: `~/.openclaw-dev/openclaw.json` had `agents.defaults.model.primary: "hydra/claude-opus-4.6"` which Hydra exposes as `input_modalities: ["text"]` (NO image input despite `type: "vision"`); fixed by switching primary to `hydra/gpt-5.4` (verified `["text", "image"]` via Hydra `/v1/models`). Live "Привет" answer ≤5s confirmed post-fix. | 5b849584d1 | **Forward queue (in priority order)**: (1) `commitment_kernel_policy_gate_full.plan.md` — Plan-agent drafts sub-plan, then 6+ phases via slice-implementer loop; closes bugs #1 + #3 + unblocks cutover-4. (2) `commitment_kernel_cutover3_artifacts.plan.md` — Plan-agent drafts, then phases; closes bug #2 + structurally fixes PDF/DOCX/code-patch routing via affordance. (3) Search-Composer Phase 4c — flip `intent-contractor-impl.ts:472` allowlist to include `web_research` family + live verify. After all three: orchestrator demo-ready end-to-end. |
| 2026-05-06 | **Slice F COMPLETE — B7 CLOSED** (PRs #181 / #182 / #183 / #184 / #185 / #186 / #187 + sub-plan #180). Per-`IdentityId` `TaskLedger` keyed on slice D's `IdentityId` and integrated with slice E's `MemoryStore` for cross-`/new` task tracking. Phases: P1 audit (#181, `cb379357bc` — `TargetRef` extension recommendation NO; `EpisodicEffectFamily` task slot does NOT exist, Phase 2 adds; zero pre-existing `task.*` emitters), P2 types + Episodic extension (#182, `c79d83d65d` — `TaskId` brand, `TaskRecord`, `TaskLedger` interface, 5×5 state-machine matrix, additively extends `EpisodicEffectFamily` with `task` variant + 4 lifecycle payloads), P3 `InMemoryTaskLedger` (#184, `3a4170d8ed` — Map-backed, identity isolation, idempotent cancel/complete, illegal transitions throw), P4 `SqliteTaskLedger` (#183, `64090927bd` — `node:sqlite` `DatabaseSync`, schema_version=1, idempotent migration, per-`IdentityId` predicates, separate DB path `<configDir>/task/identity-task-ledger.sqlite`), P5 commitment-runtime task hook + production wiring (#185, `9e3dd386a6` — `recordTaskOnCommitmentSatisfied` sibling of `memory-write-on-satisfied.ts`, Strategy A from audit §5: `memory-wiring.ts` fans out BOTH memory + task hooks from single `onAttestation`; `MemoryRuntime` extended with `taskLedger`; cross-reference invariant — every successful ledger write produces matching episodic call with same `taskId`+`identityId`; failure isolation — partial write tolerated, commitment satisfies regardless), P6 `<active_tasks>` recall block in IntentContractor (#186, `54efac953d` — additive optional `taskLedger?: TaskLedger` constructor dep mirroring slice E PR-#168 pattern; `<active_tasks>` block prepended before `<memory>`; recall failure → `task_recall_failed` uncertainty tag + warn + never throws; combined memory+tasks both blocks emit when both deps populated; structural query — no raw user text), P7 B7 acceptance fixture (#187, `5b849584d1` — 5 cases: positive B7 closure across simulated `/new`, reverse omit-`taskLedger`, reverse anonymous, reverse `commitmentSatisfied=false`, IdentityRegistry resolution; fail-first probe verified by commenting out `taskLedger` spread at `run-turn-decision.ts:357` reverts positive to absence-of-fix). **Frozen-layer integrity preserved across all 7 phases**: `src/platform/commitment/` saw exactly ONE additive change (`intent-contractor-impl.ts` constructor extension in P6, mirroring slice E P6 precedent); 5 frozen contracts byte-identical. **Slice F status: COMPLETE.** Foundation for Slice G (subagent registry), Slice J (artifacts), Slice K (reminders) — they consume the now-typed-and-wired `task.*` `EpisodicMemoryEvent` family + can cross-reference `TaskLedger` rows. All 7 PRs admin-merged via blanket maintainer-signoff. | 5b849584d1 | Slice G (subagent registry) and Slice J (artifacts) are next on the v1 roadmap; Slice H Phase 5 stress + Phase 6 live-verify still deferred (non-blockers for v1 demo). |
| 2026-05-05 | Memory bootstrap memoization fix (PR-#177) + legacy memory-tool EBUSY suppression (PR-#178). PR-#177: replaces `Singleton.cfg` reference key with `Singleton.signature` (stable JSON-stringified hash of `agents` + `identities` + resolved `stateDir`); root cause was `structuredClone(params.config)` in `cli/command-secret-gateway.ts:332` re-cloning cfg per turn → cfg-ref always missed → bootstrap rebuilt 4× per Telegram turn. New `computeMemoryRuntimeSignature` defense-in-depth catches non-serializable cfg via unique sentinel (invariant #15). 10/10 tests pass incl. central stress case `4 deep-clones → 1 bootstrap`. PR-#178: legacy memory-tool sqlite backup-rename hits EBUSY on Windows when `dev.sqlite` is open elsewhere (slice-E or pre-existing handle). Selective `reportLegacyMemorySyncFailure(logger, reason, err)` helper detects `code === "EBUSY" && syscall === "rename"` (with `.backup-` message-text fallback) and demotes to debug; non-EBUSY errors at the 4 callsites (`runDetachedMemorySync` / `warmSession` / `search` / `processSessionDeltaBatch`) STILL surface as warn (regression-safe). 10 new tests + 320/320 src/memory regression. Both admin-merged via blanket maintainer-signoff. | 0a87e35f97 | Live re-test (memory bootstrap should fire 1× per gateway lifetime; EBUSY warns silenced). Then optional sqlite-vec embedder config in `~/.openclaw/openclaw.json` for cross-restart B1 persistence (provider choice pending user input). |
| 2026-05-05 | **Slice I rollback + principled B5 fix (PR-#174 + PR-#175).** Live test on 2026-05-05 19:21-19:24 surfaced two issues: (a) `english_meta_*` regex post-filter (PR-#162) violates master plan §3 ("LLM-mediated, not regex") AND fails on mid-line / non-verb-prefix leaks (e.g., "I should save this to memory" mid-paragraph; "Let me first check"); (b) sanitizer at `deliver.ts:404` only fires on Path B consolidated final replies — Path A streaming preamble (`agent-runner.ts:866` raw `onBlockReply` bypass) leaks unsanitized. **Fix architecture**: drop the regex layer entirely; use **Anthropic Extended Thinking API** (`thinking: { type: "enabled", budget_tokens: 8000 }`) — model's reasoning lands in a structured `type: "thinking"` content block that existing `dropThinkingBlocks` (`pi-embedded-runner/thinking.ts`) and `extractAssistantThinking` (`pi-embedded-utils.ts:306`) already strip on the extraction path BEFORE both Path A and Path B emit. PR-#174 (revert): drops 7 `english_meta_*` regex entries from `OUTBOUND_LEAK_PATTERNS`; deletes `outbound-sanitizer.english-meta.test.ts`; rewrites policy + B5-replay tests to drop regex assertions; 16 diagnostic patterns BYTE-IDENTICAL; full outbound suite 515 pass / 2 skipped / 48 files. PR-#175 (extended thinking): NEW `src/agents/pi-embedded-runner/anthropic-thinking-wrapper.ts` (`createAnthropicThinkingWrapper(baseStreamFn, opts)`) + 23-case test; gates `payload.thinking = { type: "enabled", budget_tokens: 8000 }` on `model.api === "anthropic-messages" && runtimeChannel ∈ EXTERNAL_DELIVERY_SURFACES` (telegram/whatsapp/signal/slack/discord/sms/voice/imessage/googlechat); webchat + non-Anthropic models pass through unchanged. Wired in `attempt.ts:2471-2483` after `applyExtraParamsToAgent`. Composes cleanly with `createAnthropicToolPayloadCompatibilityWrapper`. Phase 4 prompt hint (`internal-reasoning-hint.ts`) KEPT as defense-in-depth fallback. **Slice I status: re-CLOSED on principled architecture** (regex layer removed; B5 closure rests on API-level extended thinking + existing tag-strip + Phase 4 advisory hint). Both admin-merged via blanket maintainer-signoff. | 8e7473593e | Restart gateway + live re-verify (Vladimir sends same 4 prompts; expect zero `[outbound-sanitizer]` events for `english_meta_*` since patterns are gone, expect zero plaintext English meta-leak in Telegram replies since extended thinking routes reasoning to API blocks). |
| 2026-05-05 | **Slice E gateway wiring bridge (PR-#172) — production live demo unblocked.** Closes the last-mile gap between Phase 7's fixture-mode acceptance test and the real gateway: at boot, constructs `SqliteVecMemoryStore` singleton (or degraded `InMemoryMemoryStore` if memory-config absent / embedder unavailable / sqlite-vec extension load fails / `cfg.identities` malformed — every path catches + downgrades + warns, NEVER throws into the gateway per invariant #15); per-turn, resolves `IdentityId` via `resolveIdentityFromSessionKey(registry, sessionKey)` from the existing slice-D `IdentityRegistry`; spreads `{memoryStore, identityId, memoryLogger, onAttestation}` into `RunTurnDecisionInput` so Phase-6 `<memory>` recall fires AND Phase-5 `recordMemoryOnCommitmentSatisfied` writes on `commitmentSatisfied=true`. NEW `extensions/AUDIT-slice-e-gateway-wiring.md` (262 LOC read-only audit). NEW `src/server/memory-store-bootstrap.ts` (243 LOC) + tests (8 cases: SqliteVec branch with embedder probe, three downgrade paths, cfg-reference memoization, cfg-reload rebuild, identity-registry happy + anonymous-default). NEW `src/platform/decision/memory-wiring.ts` (124 LOC) + tests (9 cases: empty/whitespace sessionKey, anonymous, wrapped scopes, identity resolution happy, episodic+semantic write on satisfied, reverse on unsatisfied, anonymous shape, store failure absorbed). MOD `src/platform/decision/input.ts` (+22 LOC, under ≤30 cap), `src/agents/agent-command.ts` (+11), `src/auto-reply/reply/agent-runner-utils.ts` (+4), `src/gateway/server-startup.ts` (+10, under ≤80 cap). Boot site: `src/gateway/server-startup.ts:80 startGatewaySidecars(...)`; per-turn chain: `<channel>` → `agent-runner-execution.ts:146 resolveRoutingSnapshotForTemplateRun` (auto-reply) OR `agent-command.ts:1280 prepareAgentCommandExecution` (CLI) → `buildClassifiedExecutionDecisionInput` → `runTurnDecision`. Gate (authoritative — agent sandbox blocked local install): `pnpm exec tsgo --noEmit` exit 0; targeted 17/17 (8 bootstrap + 9 wiring); `src/platform/decision/` 206/206 (197 baseline + 9 new); `src/platform/memory/` 138/138 (no regression); `src/server/` 8/8; pre-existing failure counts in `pi-embedded-runner` (47), `auto-reply/reply/` (54), `agents/` (572), `gateway/` (104) unchanged from dev baseline — NO new regressions. Frozen layer untouched (`src/platform/commitment/**` byte-identical). All 6 behaviour invariants verified verbatim by gate-runner: (1) no-embedder → InMemory, (2) embedder-throw → InMemory + warn, (3) sqlite-vec-throw → InMemory + warn, (4) malformed identities → InMemory + warn, (5) anonymous → empty wiring (no recall, no write), (6) commitmentSatisfied=false → no-op. Admin-merged via blanket maintainer-signoff. **Working smart orchestrator: B1 (memory persists across `/new`) + B4 (stream/edit ack-ordering) + B5 (English meta-leak) all closed and wired in production. Live demo achievable.** | 158e4c0f7c | Slice F (TaskLedger), Slice G (subagent registry), Slice J (artifacts), Slice K (reminders) — sub-plans not yet drafted; consume `MemoryStore` keyed on `IdentityId`. Slice H Phase 5 stress + Phase 6 live verify deferred (non-blocker for v1 demo). |
| 2026-05-05 | **Slice E Phase 7 (PR-#170) — Slice E COMPLETE; B1 CLOSED.** End-to-end B1 replay acceptance fixture + production wiring at `run-turn-decision.ts`. NEW `src/platform/memory/b1-replay.acceptance.test.ts` (493 LOC, 5 cases): positive B1 closure (turn-2 surfaces reminder planted in turn-1 across simulated `/new` boundary using real `SqliteVecMemoryStore` against tmp-dir sqlite file, contractor closed+reopened with fresh instance pointing at same DB path), anonymous-session negative, `commitmentSatisfied=false` reverse, **omit-memoryStore reverse-test** (proves wiring is what carries the closure), IdentityRegistry resolution smoke. MODIFIED `src/platform/decision/run-turn-decision.ts` (+37 LOC) — wired optional `memoryStore` / `identityId` / `memoryLogger` on `RunTurnDecisionInput`; spread into `createIntentContractor(...)` inside `runShadowBranch`. ~40 LOC wiring (under 50-LOC cap). Frozen layer untouched (`src/platform/commitment/` source byte-identical; wiring lives in non-frozen `src/platform/decision/`). `pnpm exec tsgo --noEmit` exit 0; `src/platform/memory/` 138/138 (133+5); `src/platform/decision/` 197/197 (no regression). Hard invariants #5/#6/#8/#11/#15. Admin-merged via blanket maintainer-signoff. **Slice E status: COMPLETE** (P1 #154, P2 #156, P3 #160, P4 #165, P5 #169, P6 #168, P7 #170). Foundation for slices F/G/J/K. Out-of-scope follow-ups: `input.ts` → gateway plumbing; cron-family emit at slice J's site; live-verify replay (v1-release acceptance §5). | f4cc3af92d | Slice F (TaskLedger) and Slice G (subagent registry) are the next two slices toward v1 release; both consume `MemoryStore` keyed on `IdentityId`. Slice H Phase 5 (stress) deferred — non-blocker. |
| 2026-05-05 | Slice E Phase 6 (PR-#168) — IntentContractor optional `memoryStore` dep + `<memory>` block (frozen-layer ADDITIVE). MODIFIED `src/platform/commitment/intent-contractor-impl.ts` (+175 -4) — `createIntentContractor` constructor accepts optional `memoryStore?: MemoryStore`, `identityId?: IdentityId`, `logger?: IntentContractorLogger`, `memoryRecallLimit?: number`. Recall fires at lines ~233-243 inside `classify` BEFORE `adapter.classify(...)`. Top-K default = 5. Empty result → no block. Failure → no block + `memory_recall_failed` uncertainty tag + warn; NEVER throws. NEW `intent-contractor-impl.memory-recall.test.ts` (415 LOC, 11 cases). Block format = `<memory>${JSON.stringify({entries})}</memory>` (closed-shape JSON twin of `<web_evidence>`; cross-import from decision into commitment would violate #8, so the builder is a 4-line near-twin). Existing 13 contractor tests unchanged byte-identically (regression baseline preserved). 5 frozen contracts untouched. Constructor change is ADDITIVE — existing 2-arg callers compile + behave byte-identical. `pnpm exec tsgo --noEmit` exit 0; targeted 24/24; `src/platform/commitment/` 129/129 (no regression); `src/platform/decision/` 197/197. Admin-merged via blanket maintainer-signoff. | 883256a2b1 | Phase 7 — B1 replay end-to-end acceptance fixture + wiring `memoryStore`/`identityId` from `run-turn-decision.ts` into the contractor. |
| 2026-05-05 | Slice E Phase 5 (PR-#169) — commitment-satisfied memory write hook. NEW `src/agents/pi-embedded-runner/run/memory-write-on-satisfied.ts` (252 LOC) — `recordMemoryOnCommitmentSatisfied(...)` returns `{kind:'wrote'|'skipped'|'failed', ...}`; reads attestation via structural type `CommitmentSatisfiedAttestationLike`, NOT via import from `src/platform/commitment/` (defense-in-depth around invariant #8); future effect-family slots (`subagent.created`/`reminder.set`/`artifact.created`) typed-but-inert. NEW test (381 LOC, 11 cases). MODIFIED `src/platform/decision/run-turn-decision.ts` (+40 LOC) — added optional `onAttestation` callback on `RunTurnDecisionInput`; existing call sites pass `undefined`. Audit deviation: sub-plan §2.5 said the existing call site lived in `src/agents/pi-embedded-runner/run/`; actual code routes attestation only through the decision pipeline (`run-turn-decision.ts:evaluateCutoverGate` is the sole production caller of `MonitoredRuntime.run(...)`). Hook FILE stays in `src/agents/pi-embedded-runner/run/` per spec; joining seam is the `onAttestation` callback so hook stays out of frozen layer. Frozen layer source untouched (no import from `src/platform/commitment/`; 5 frozen contracts byte-identical). `pnpm exec tsgo --noEmit` exit 0; targeted 11/11; decision module 197/197; memory module 133/133; pi-embedded-runner 451 pass / 47 pre-existing fail (count identical to dev baseline — no new regressions; pre-existing `TypeError: this._modelRegistry.getApiKey is not a function`). Admin-merged via blanket maintainer-signoff. | 24ed296923 | Phase 6 — IntentContractor recall hook (constructor accepts optional `memoryStore?: MemoryStore`). |
| 2026-05-05 | **Slice I Phase 5 + Phase 6 (PR-#166) — Slice I CLOSED.** Policy-aware sanitizer wiring + B5 replay acceptance fixture. MODIFIED `src/infra/outbound/outbound-sanitizer.ts` — new optional `policy?: ReplySanitizerPolicy` arg with frozen default `{ reasoning: "strip" }`; structured branch wraps english_meta_* matches in `<thinking lang="en">{escaped}</thinking>` (HTML-escapes `<`/`>`/`&`); deferred=strip in v1; existing 16 patterns BYTE-IDENTICAL across every policy. Telemetry preserved for both strip and wrap. MODIFIED `src/infra/outbound/deliver.ts` — gate expanded to `isReplySanitizerSurface(channel)`; passes `resolveReplySanitizerPolicy(channel)` to sanitizer. NEW `outbound-sanitizer.policy.test.ts` (+402, ~80 cases incl. cross-policy fuzz × 20 combos); NEW `deliver.b5-replay.test.ts` (+310, 5 cases: telegram strip, stress, slack deferred, webchat structured wrap, gate property). `pnpm exec tsgo --noEmit` exit 0; `pnpm exec vitest run src/infra/outbound/` 49 files / 582 pass / 2 skipped. Hard invariants #5/#6/#8/#11/#15 reverse-tested. Closes B5 (English meta-text leak from 2026-05-04 Telegram session). Admin-merged via blanket maintainer-signoff. **Slice I status: COMPLETE** (P1 #152, P2 #157, P3 #162, P4 #161, P5+P6 this PR). | 18033493235b | Slice E Phase 5 (commitment-runtime memory hook) and Phase 6 (IntentContractor recall) are the next two unblocked slices toward B1 closure. |
| 2026-05-05 | Slice E Phase 4 (PR-#165) — `LlmExtractorMemoryStore` (Path B in-house extractor; sandbox blocked Node-SDK audit of mem0ai per sub-plan §2.2 fallback authorisation). NEW `src/platform/memory/llm-extractor-prompt.ts` + tests (frozen prompt + Zod `{keep, normalized, tags}` schema + code-fence-tolerant parser). NEW `src/platform/memory/llm-extractor-store.ts` + tests (~22 cases: interface conformance, extraction-on/off round-trip, extractor-crash → passthrough + warning, identity isolation, **pluggability** — Phase 2 acceptance suite passes against this store with constructor swap). MODIFIED `src/platform/memory/index.ts` (barrel). LLM provider not bound by this module — injected at call site (Phase 5 will wire existing memory-config). `package.json` NOT modified. `pnpm exec tsgo --noEmit` 0 errors; `pnpm exec vitest run src/platform/memory/` 8 files / 133 tests pass. Hard invariants #5/#6/#8/#11/#15. Admin-merged via blanket maintainer-signoff. §6 D1 amendment (mem0 vs in-house) flagged as non-blocking follow-up for next roadmap revision. | 09c9f06a01 | Phase 5 — commitment-runtime memory hook on `commitmentSatisfied=true` (lives outside frozen `src/platform/commitment/` per invariant #8). |
| 2026-05-05 | Slice E Phase 3 (PR-#160) — `SqliteVecMemoryStore` persistent backend. NEW `src/platform/memory/sqlite-vec-store.ts` (707 LOC) + tests (414 LOC, 13 cases against real tmp-dir sqlite). Schema `memory_episodic` + `memory_semantic` + `vec0` virtual table; idempotent `CREATE TABLE IF NOT EXISTS` + `schema_version=1`. Injection seams `MemoryEmbedder` / `SqliteVecMemoryStoreLogger` / `LoadSqliteVecExtensionFn` keep the store free of provider-specific imports. Identity isolation structural — every read predicates on `identity_id = ?`, vec0 omits identity by design, recall JOIN re-applies the filter. Graceful fail when extension load returns `{ok:false}`: surfaces `vector_unavailable` warning exactly once on the injected logger, falls back to LIKE-based recall, base tables preserved. Idempotency verified across three sequential opens. `pnpm tsgo` 0 errors; vitest 106/106 across `src/platform/memory/` (23 in-memory + 13 sqlite-vec + 70 Phase-1 contract). Hard invariants #5/#6/#8/#11/#16 reverse-tested. Admin-merged via blanket maintainer-signoff (Vladimir 2026-05-05). | d1fa5de6cb | Phase 4 — mem0 wrapper or in-house extractor (pluggability test must pass against both impls swapped at construction). |
| 2026-05-05 | Slice I Phase 4 (PR-#161) — `internalReasoningHint` advisory system-prompt section. NEW `src/agents/pi-embedded-runner/internal-reasoning-hint.ts` (constant + predicate single source of truth) + 19-case test. MODIFIED `src/agents/system-prompt.ts` (new optional param + `## Internal Reasoning Format` section between existing `reasoningHint` block and next heading), `system-prompt.test.ts` (5 golden tests), `pi-embedded-runner/system-prompt.ts` (param threading), `pi-embedded-runner/run/attempt.ts` (computes hint at line ~2067 alongside `reasoningTagHint`). Triggers ONLY when `isReasoningTagProvider(provider) === false` AND `runtimeChannel ∈ EXTERNAL_DELIVERY_SURFACES`. Advisory only — does not gate output, distinct from strict `<think>+<final>` block reserved for google/minimax. `pnpm tsgo` clean; vitest 72/72 across the three named test files; affected suite 31/31 (no regressions). Hard invariants #5/#6/#8/#11/#15. Admin-merged via blanket maintainer-signoff. | 190381adc0 | Phase 5 (policy wiring at `deliver.ts:404`) is now unblocked — Phase 2 policy resolver + Phase 3 patterns + Phase 4 prompt-side hint all merged. |
| 2026-05-05 | Slice I Phase 3 (PR-#162) — `english_meta_*` pattern family extension to `outbound-sanitizer.ts`. MODIFIED `src/infra/outbound/outbound-sanitizer.ts` (+97 -3) — adds 7 new patterns (`english_meta_let_me`, `_ill`, `_i_should`, `_first_ill`, `_lets`, `_looking_at`, `_checking`) to `OUTBOUND_LEAK_PATTERNS`; existing 16 patterns BYTE-IDENTICAL. NEW `outbound-sanitizer.english-meta.test.ts` (+361, 37 cases): pattern-coverage + positive + negative + code-block-protected + regression + clean passthrough + telemetry. Reuses existing `findCodeRegions` / `isInsideCode` at `src/shared/text/code-regions.ts`. `pnpm tsgo` clean; targeted vitest 64/64; full outbound regression 486 pass / 2 skipped / 46 files. Hard invariants #5/#6/#8/#15 (defense-in-depth — patterns are post-filter; prompt-side defense in Phase 4). Admin-merged via blanket maintainer-signoff. | ac77fcdf8a | Phase 5 — wire `policy.reasoning` parameter at `deliver.ts:404` to route patterns to `strip` (telegram/etc.) vs `structured` (webchat) vs `deferred` (slack/discord). |
| 2026-05-05 | Slice H Phase 3 + Phase 4 (PR-#163) — H2 sentinel-entry fix for B4 ack-ordering. **Chosen path**: H2 sentinel (option (a) per sub-plan §3) over new finalize-kind (option (b)) — sentinel uses internal `Symbol` field on the deferred entry; two existing iteration sites (merge + replay) reorder to position 0. ~50 LOC `block-external-buffer.ts` + ~15 LOC `agent-runner.ts`, well inside §6 surface-cap budget; preserves T1–T6 regression contract verbatim by gating sentinel logic on symbol presence. MODIFIED `src/auto-reply/reply/block-external-buffer.ts` (`ACK_SENTINEL` + `enqueueAck` + sentinel-aware merge/replay), `src/auto-reply/reply/agent-runner.ts` (`emitDeferredAck` re-routed to `deferral.enqueueAck()` when deferral active, replacing raw `effectiveOpts?.onBlockReply` bypass at line 866), `block-external-buffer.ack-ordering.test.ts` (un-marked `it.fails` → `it`; +3 new cases — replay branch, negative coverage, idempotency). NEW `block-external-buffer.b4-fixture.test.ts` — Phase-4 B4 transcript replay producing single consolidated send and zero delete ops. Sub-plan-prose path was `src/agents/agent-runner.ts`; ACTUAL path is `src/auto-reply/reply/agent-runner.ts` (kickoff sketch was off; §2 audit stayed accurate). `pnpm tsgo` 0 errors; vitest 13/13 across the three ack-ordering files. The previously-failing `it.fails` test now passes as `it(...)` — H2 fix closes B4. Hard invariants #5/#6/#8/#11/#15 (defense-in-depth at the deferral seam). Admin-merged via blanket maintainer-signoff (HTTP 504 on first attempt; second attempt succeeded). Phase 5 (stress tests) and H3 secondary (channel-adapter delete-archive race at `bot-message-dispatch.ts:235`) deferred. | 5c74fd2197 | Slice E Phase 4 (mem0 wrapper) and Slice I Phase 5 (policy wiring) are the next two unblocked v1 slices. |
| 2026-05-05 | Slice E Phase 2 (PR-#156) — `InMemoryMemoryStore` (Map-backed, no I/O) Phase-2 impl satisfying the Phase-1 `MemoryStore` interface so slices F/G/J can integrate before persistent backend lands. New `src/platform/memory/in-memory-store.ts` (+429); new `src/platform/memory/in-memory-store.test.ts` (+357, 23 cases) covering episodic/semantic round-trip, identity isolation (`identity:vladimir` not visible from `identity:alice` on the same store), `forget` removal, `recall` returns empty (NOT throw) on no match, `list` paginated stable-ordered, schema validation rejection. Modified `src/platform/memory/index.ts` (+5) to export the impl. Gate clean: `pnpm tsgo` 0 errors; scoped vitest 23/23. Hard invariants #5/#6/#8/#11/#16 reverse-tested; frozen layer untouched. Admin-merged via `gh pr merge 156 --admin --squash --delete-branch`. | c81af822fc | Phase 3 — `SqliteVecMemoryStore` persistent backend with `sqlite-vec` extension + graceful fail to LIKE-based recall. |
| 2026-05-05 | Slice I Phase 2 (PR-#157) — channel-keyed sanitizer policy types. New `src/infra/outbound/reply-sanitizer-policy.ts` (+148) with `ReplySanitizerPolicy = { reasoning: "strip" | "structured" | "deferred" }`, frozen-singleton `resolveReplySanitizerPolicy(channel)`, predicate `isReplySanitizerSurface`, and superset `REPLY_SANITIZER_SURFACES` covering `CHAT_CHANNEL_ORDER` ∪ `EXTERNAL_DELIVERY_SURFACE_LIST` ∪ `{INTERNAL_MESSAGE_CHANNEL}` per Phase 1 audit. Mapping: telegram/whatsapp/signal/imessage/googlechat/line/irc/max/voice/sms → `strip`; webchat → `structured`; slack/discord → `deferred`. Unknown channel → `strip` (defense-against-leak default; MUST NEVER default to `structured`). Tests `src/infra/outbound/reply-sanitizer-policy.test.ts` (+131, 26 cases) over 3 describe blocks (resolver mappings, surface inventory, predicate); fail-first verified. No call-site wiring (Phase 5 will wire `deliver.ts:404`). `pnpm tsgo` clean; vitest 26/26. Hard invariants #5/#6/#8/#11/#15 reverse-tested; frozen layer untouched. Admin-merged via `gh pr merge 157 --admin --squash --delete-branch`. | 6dcba2258e | Phase 3 (`english_meta_*` pattern family) — **REQUIRES MAINTAINER SIGNOFF** per sub-plan §0 todo + §6.2. Phase 4 (system-prompt hint) similarly gated. |
| 2026-05-05 | Slice H Phase 2 handoff doc (PR-#158) — carry-over of canonical handoff entry for slice H Phase 2 (PR-#155) that was prepared in a prior session but not committed before token limit. Master §0 row + sub-plan todo flip only; no code changes. Admin-merged via `gh pr merge 158 --admin --squash --delete-branch`. | 69f278844e | Slice H Phase 3 still gated on maintainer signoff per sub-plan §0. |
| 2026-05-05 | Slice H Phase 2 (PR-#155) — failing harness/test reproducing B4 ack-ordering at the deferral seam. New file `src/auto-reply/reply/block-external-buffer.ack-ordering.test.ts` (+231 LOC, test-only); production code untouched. Drives real `createBlockReplyPipeline` + `createExternalBlockReplyDeferral` through the deferral wiring (no `vi.spyOn` on function under test); mock channel-adapter recorder is the only stub. Scenario mirrors `agent-runner.ts:592–613` (wiring) / `:850–873` (`emitDeferredAck`) / `:988–996` (`finalizeAfterRun`): 2 partial deltas → `notifyStructuralToolExecutionStarting()` → ack via the bypass path replicating `agent-runner.ts:866` (`effectiveOpts?.onBlockReply`, raw — NOT the wrapped deferral) → final delta → `pipeline.flush({force:true})` → `finalizeAfterRun(streamingAwareBlockReply)`. Encoded with vitest's `it.fails(...)` so CI stays green while the bug is documented; **fail-first verified on dev HEAD `67d3408a30`** by temporarily swapping `it.fails`→`it` — verbatim failure `AssertionError: expected 1 to be +0 // Object.is equality` (`Expected: 0`, `Received: 1`). Recorded ordering on dev HEAD = `[send-ack, send-consolidated]`, mapping to sub-plan §3 case `[ack_send, preamble_send, final_send] → **H2 (medium-high primary)**` confirmed. Phase 3 will commit to the H2 fix (ack as first deferred entry with `isDeferredAck` sentinel OR ack routed through the deferral wrap with a 4th finalize-kind `ack-then-consolidate`). Negative coverage included (regression guard for the no-tool-no-ack replay path). `pnpm tsgo` clean; 1 expected fail + 1 passed scoped vitest. Hard invariants #5, #11, #15 reverse-tested; frozen layer untouched. Admin-merged via `gh pr merge 155 --admin --squash --delete-branch` per documented BlackSmith precedent. | 9d12445530 | Phase 3 — pick + apply H2 fix; **requires explicit maintainer signoff** per sub-plan §0 maintainer-signoff field. Surface caps from §6: ≈30–60 LOC `agent-runner.ts` + ~20 LOC `block-external-buffer.ts` + tests. Phase 3 will un-mark `it.fails` on the new test. |
...
| 2026-05-06 | **Cutover-3 Artifacts SLICE COMPLETE** (PRs #194/#195/#197/#198/#200/#202/#205/#207 — Phase 1 audit + 8 phases: artifact effect-family + 4 effect ids in `EFFECT_FAMILY_REGISTRY`; `WorldStateSnapshot.artifacts` slice + `ArtifactWorldStateObserver` (process singleton); 4 affordances `pdf.created`/`docx.created`/`code_patch.applied`/`image.created` + 4 done-predicates over `state.artifacts.records`; runtime adapter `artifact-runtime-adapter.ts` + 4 emit sites at pdf-tool/docx-tool/image-generate-tool/apply_patch + `recordArtifactOnCommitmentSatisfied` hook (slice E `artifact` payload slot now LIT first time on dev); `inboundMediaResolver?` constructor extension on IntentContractor + `<inbound_attachments>` block + `INBOUND_IMAGE_REFERENCE_AVAILABLE_PRECONDITION` resolver + tool-arg pre-binding (closes bug #2 img2img architecturally — NOT per-provider hack; structural precondition through AffordanceRegistry); `CUTOVER_2` extended with all 4 artifact effect ids; ambient-turn singleton wired in `attempt.ts` try/finally; classifier prompt-hint allowlist gained `artifact` family; 11 acceptance test cases). **PolicyGate Full SLICE COMPLETE** (PRs #196/#199/#201/#203/#204/#206/#208/#209/#210 — Phase 1 audit + 10 phases: 5 orthogonal `*POLICY_REASONS` tuples (`APPROVAL_/BUDGET_/ROLE_/RETRY_/ESCALATION_POLICY_REASONS`) + `EpisodicEffectFamily` extended with 5 `policy_*` arms; Stages 2-6 impls (`createApprovalPolicy`/`createBudgetPolicy`+`SqliteBudgetStore`/`createRolePolicy`/`createRetryPolicy`+LRU state store/`createEscalationHook`); evaluation chain `affordance → approval → budget → role → retry` in `runTurnDecision` with skip-on-prior-denial + escalation fan-out; bug #1 `closure-outcome-dispatcher` discriminated `EnsureBootstrapRequestsResult` + capability-already-verified early-return; bug #3 `SESSIONS_YIELD_ABORT_SETTLE_TIMEOUT_MS` 2000→60000ms + config override at `agents.sessionsYieldAbortSettleTimeoutMs`; 17 acceptance fixture cases; `policy-gate.ts`+reverse-test BYTE-IDENTICAL through entire slice — orthogonal pattern). **Search-Composer pipeline COMPLETE** (PR #211 — `intent-contractor-impl.ts:916` allowlist gained `web_research` family; finalises Search-Composer Phase 4c that was blocked on PolicyGate Full + Cutover-3 completion). All 19 PRs frozen-layer additive only; 16 invariants held; 5 frozen contracts BYTE-IDENTICAL throughout. **LIVE-VERIFY result: PARTIAL — code merged correctly per spec; sub-plan acceptance fixtures green; live Telegram demo BLOCKED by 4 newly-surfaced orchestration-layer bugs (NEW-A/B/C/D — see §0.5.6).** | 735f65c019 | NEW-A/B/C/D triage (modality-aware routing / unified `/new` reset / outbound coalescer / locale-aware sanitizer); cutover-4 (`repo_operation.completed`) now unblocked. |
| 2026-05-07 | **IntentContractor freshness/recency SLICE COMPLETE** (PRs #266/#267/#268/#269/#270/#271 — sub-plan landing + 5 phases: Phase 1 audit (`extensions/AUDIT-intent-contractor-freshness.md`, 561 LOC; identified `buildMemoryBlock` no-sort + `buildActiveTasksBlock` ignores `updatedAt` + `metadata.recordedAt` not written today + single `Date.now()` site at `intent-contractor-impl.ts:712` + DECAY_FLOOR floor recommendation); Phase 2 types (NEW `src/platform/freshness/freshness-config.ts` — `FreshnessConfig` + `ResolvedFreshnessConfig` + `RecencyScored<T>` + `DECAY_FLOOR=0.05` + `DEFAULT_HALF_LIFE_MS=7d` + `DEFAULT_WINDOW_MS=30d` + `FreshnessConfigSchema.strict()` + `resolveFreshnessConfig()`; 24/24 tests; sibling-module placement); Phase 3 pure `scoreByRecency<T>` helper (NEW `src/platform/freshness/score-by-recency.ts`; clock-skew clamp + DECAY_FLOOR floor + stable sort; 15 tests; fail-first verified); Phase 4 wiring (additive `now?` + `freshnessConfig?` deps on `createIntentContractor`; `<memory>` recency reorder via `combinedScore = score * recencyDecay`; `<active_tasks>` reorder by `updatedAt ?? createdAt`; NEW `buildFreshnessHintsBlock`; producer-side `metadata.recordedAt` write in `LlmExtractorMemoryStore.mergeMetadata` with `now?` seam + new `RECORDED_AT_METADATA_KEY` export; 22 new tests; 1309/1309 regression green; sha256-verified frozen layer); Phase 5 acceptance + log emissions (`emitFreshnessLogLines` helper emits 4 structured log lines `[intent-contractor] freshness.applied / memory.block / active_tasks.block / freshness_hints.block`; high-N cohort acceptance test; NEW `extensions/RUNBOOK-freshness-live-verify.md` operator runbook; sub-plan flipped to `completed`). Behaviour-changing on production for any turn with memory cohort > 10 entries — fresher memory ranks higher in `<memory>` block; recently-updated tasks rank above older-created tasks. **6 PRs merged this session for freshness slice; frozen-layer additively touched only; 16 invariants preserved; 5 frozen contracts BYTE-IDENTICAL throughout.** Master plan §0.5.6 NEW-A/B/C/D rows confirmed `closed`; freshness slice CLOSED autonomously. | 03eaad5ddb | Operator live-verify in Telegram per `extensions/RUNBOOK-freshness-live-verify.md`. Master-plan deferred items remain: Slice H P6 live-verify (operator), Bug F persistent worker push (no sub-plan yet), concurrent broker (PR-MT, signoff), bundle-as-contract (signoff). |
| 2026-05-07 | **Bug F persistent-worker subsequent push SLICE COMPLETE** (PRs #273 (sub-plan landing) / #274 (Phase 1 audit) / #275 (Phase 2 types) / #276 (Phase 3 affordance + done-predicate) / #277 closed → #278 (Phase 4 runtime adapter) / #279 (Phase 5 cron-fire callback + WorldState slice + observer) / #280 (Phase 5b spawn-site `ownerIdentityId` wiring + `subagent_ended` companion emitter) / #281 (Phase 5c bootstrap binder) / #282 (Phase 5d production transport closure) / #283 (Phase 6 OutboundCoalescer bypass-reason removal) / this PR (Phase 7 acceptance + runbook + sub-plan flip + master plan entry) — sub-plan landing + 7 phases: Phase 1 audit (`extensions/AUDIT-persistent-worker-push.md` mapped cron-fire seam at `cron/isolated-agent/run.ts:619-680`, `subagent_ended` lifecycle as canonical close-of-run signal, slot E2 `cron_persistent_worker` bypass in OutboundCoalescer, slice K `wrappedScopeIdentityId` injection precedent, slice E `EpisodicMemoryEvent.persistent_worker.push` confirmed NEW effect not STUB-light); Phase 2 types (NEW `src/platform/persistent-worker/persistent-worker-push-types.ts` — `PersistentWorkerPushTriggerId` brand + `WorkerReportRef` closed shape + `PERSISTENT_WORKER_SUBSEQUENT_PUSH_EFFECT` + `WORKER_REPORT_AVAILABLE_PRECONDITION` + `WORKER_REPORT_CONTENT_MAX_LENGTH=4096` + `WorkerReportRefSchema.strict()` Zod; tests round-trip + brand discipline + reject empty/malformed/oversized + identity-mismatch reverse); Phase 3 affordance + done-predicate (REUSE `COMMUNICATION_EFFECT_FAMILY`; NEW `PERSISTENT_WORKER_SUBSEQUENT_PUSH_AFFORDANCE_ENTRY` registered on default `AffordanceRegistry` — id `persistent_worker.subsequent_push`, target matcher `external_channel|unspecified`, requiredPreconditions `[IDENTITY_RESOLVED, WORKER_REPORT_AVAILABLE]`, riskTier `medium`, `defaultBudgets={maxLatencyMs:15_000, maxRetries:0}`; NEW `done-predicate-persistent-worker-push.ts` reads `state.persistentWorkerReports?.delivered[]` + `expectedDelta.persistentWorkerReports?.added[]` with closed missing-key set `slice_absent` / `delivered.empty` / `worker_report_missing:<id>` / `worker_report_status_unexpected:<id>:<status>`); Phase 4 runtime adapter (`runPersistentWorkerSubsequentPush` — closed 8-entry failure set `transport_error` / `identity_unavailable` / `channel_invalid` / `worker_run_missing` / `observer_unavailable` / `report_already_pushed` / `dispatch_failed` / `internal_error`; NEVER throws; mark-`pushed`-BEFORE-dispatch idempotency parity with slice K reminder-fire `markFired`); Phase 5 cron-fire callback + WorldState slice + observer (`persistentWorkerPushFireCallback` at `src/cron/isolated-agent/persistent-worker-push-fire-callback.ts`; NEW `WorldStateSnapshot.persistentWorkerReports?` slice + `PersistentWorkerReportObserver` + `createPersistentWorkerReportCollector` per-(sessionId, turnId) keyed with `perTurnLimit=2`; `SubagentRunRecord` ADDITIVE optional fields `ownerIdentityId` / `frozenResultText` / `subsequentPushStatus`); Phase 5b spawn-site `ownerIdentityId` wiring + `subagent_ended` companion emitter (`emitPersistentWorkerSubsequentPushIfApplicable` parallel fan-out at `subagent-registry.ts:666` — fires alongside `emitSubagentEndedHookForRun` for `spawnMode === 'session'` runs; gating predicates: spawnMode + ownerIdentityId + channel/to + frozenResultText + callback bound); Phase 5c bootstrap binder (`setProcessPersistentWorkerPushFireCallback` at server bootstrap with `getProcessPersistentWorkerReportCollector()` singleton + `runPersistentWorkerSubsequentPush` + `subagentStore` accessors); Phase 5d production transport closure (`dispatchCronDelivery`-backed `deliveryDispatch` — REUSED, no fork); Phase 6 OutboundCoalescer bypass-reason removal (`BYPASS_REASONS` tuple narrowed 7→6 — `cron_persistent_worker` REMOVED; closed-set runtime guard at `outbound-coalescer.ts:486-490` rejects the literal; coverage guard `outbound-coalescer-bypass.test.ts` + compile-time `@ts-expect-error` reverse + `event=committed` (NOT `event=bypassed`) sanctioned-path positive); Phase 7 acceptance + log-line evidence + live-verify runbook (NEW `src/platform/persistent-worker/__tests__/persistent-worker-push.acceptance.test.ts` — 16 cases over 10 describe blocks: end-to-end emitter→callback→adapter→DeliveryReceipt + done-predicate satisfies + cross-identity defense + anonymous fail-closed + worker_run_missing + dispatch_failed mark-before-dispatch + bypass closed-set rejection + frozen-layer sha256 + frozen interfaces structural + cron-fire callback NEVER reads RawUserTurn/UserPrompt; fail-first verified by neutralising Phase 5b emitter predicate; NEW `extensions/RUNBOOK-persistent-worker-push.md` operator runbook). All structural log lines emitted: `[persistent-worker-push-fire-callback] workerRunId=<...> wrappedScopeIdentityId=<...> channel=<...> result=<...>`, `[persistent-worker-push-runtime-adapter] runPersistentWorkerSubsequentPush ... result=<...>`, `[outbound-coalescer] event=committed runId=<...>` (NOT `event=bypassed reason=cron_persistent_worker`). **12 PRs merged this slice; frozen-layer additively touched only; 16 invariants preserved; 5 frozen contracts BYTE-IDENTICAL throughout (sha256 `57fc96305711690f5d75d99d63c673ee6389f624fdf7a6c750aad0dc02e624db` against predecessor SHA `a2cadfafae`).** Sub-plan flipped to `completed`; §6 Handoff Log entry written. Master-plan deferred items remaining: Slice H P6 live-verify (operator), concurrent broker (PR-MT, signoff), bundle-as-contract (signoff). | &lt;final-sha&gt; | Operator live-verify in Telegram per `extensions/RUNBOOK-persistent-worker-push.md`. |
| 2026-05-07 | **NEW-A modality-aware routing SLICE COMPLETE** (PRs #214/#216/#217/#219/#222/#223 — 6 phases: `ModalityRequirement` union + `deriveTurnModalityRequirements` + `modelCoversModalityRequirement` + `filterCandidatesByModality` with fail-open invariant + wired into `runWithModelFallback` between L716-L732 + acceptance fixture replays L750 image-bearing turn). Closes NEW-A live demo: image-bearing Telegram turns now route to `hydra/gpt-5.4` (capability-driven, NOT per-provider hack); Opus filtered out when `inputModalities=['text']` only. Provider-agnostic. **NEW-B unified session reset SLICE COMPLETE** (PRs #215/#218/#220/#221/#224/#225 — 6 phases: `SessionResetEvent`/`SessionResetSubscriber`/`SessionResetSubscriberRegistry` + `resetTurnSession()` with insertion-order determinism + per-subscriber failure isolation; 8 subscribers — primary `subscriber:followup-queue-clear` calls `clearFollowupQueue(sessionKey)` (Phase 1 audit refuted chat-history hypothesis; real leak was `FOLLOWUP_QUEUES` not cleared on user-typed `/new` while RPC `sessions.reset` did clear); wired into `session.ts:if(isNewSession)` + `src/server/session-reset-bootstrap.ts`; B1 closed RUNTIME-wise). **NEW-C outbound coalescer SLICE COMPLETE** (PRs #227/#229/#231/#234/#235/#236/#237 — 7 phases: `OutboundCoalescer` per-`(turnId, channelKey)` keyed `Map<channelKey, Map<turnId, OutboundMessage[]>>` + per-bucket watchdog (default 60s) + drop_intermediates / merge_into_final strategies + 23 emit-sites wired through coalescer + `commitOutboundOnCommitmentSatisfied` hook (3-line defense: `commitmentSatisfied=true` edge → `finalizeAfterRun` fallback → watchdog) + closed `BypassReason` union (7 entries) + coverage guard + acceptance replays 18:26+18:27 double-emit fixture). FIRST runtime gate for `Single_final_user_facing_message_per_user_turn` invariant. **NEW-D locale-aware sanitizer SLICE COMPLETE** (PRs #226/#228/#230/#232/#233 — 5 phases: additive `LocaleFilterPolicy` on `ReplySanitizerPolicy` + `CHANNEL_LOCALE_DEFAULTS` (telegram→[ru], webchat→[ru,en]) + `resolveReplySanitizerPolicyWithLocale` + `detectPredominantLocale` (mirrors `pi-embedded-subscribe.handlers.messages.ts:350-354` arithmetic byte-for-byte) + sanitizer locale-gate branch + `[outbound-sanitizer] locale_filter applied locale=en blocked=true reason=no_cyrillic channel=telegram` telemetry; replay fixture closes L2392 ALERT-style English meta on telegram→ru). **Cutover-4 repo_operation SLICE COMPLETE** (PRs #238/#239/#240/#241/#242/#243/#244/#245 — 8 phases: `repo` family + 4 effect ids (branch_created/commit_landed/merge_completed/diff_observed) + `WorldStateSnapshot.repo` slice + `RepoWorldStateObserver` + 4 affordances + 4 done-predicates + `repo-runtime-adapter.ts` + gated `runRepoCommand` + FIRST sanctioned user-facing git surface `src/agents/tools/repo-tool.ts` (closed `kind` enum + Zod-validated args, NEVER free-form shell strings) + `recordRepoOperationOnCommitmentSatisfied` hook (slice E `repo` slot now LIT first time on dev) + NEW lint rule `lint:commitment:no-direct-git-outside-adapter` (grandfathered: workspace-probe, update-runner, test-only) + PolicyGate Full integration via config-only adds (mutation idempotency lock via Zod superRefine: `policy.retry.repo.<mutation>.maxAttempts=0` cannot be overridden) + `CUTOVER_2` flipped 8→12 entries + classifier prompt-hint allowlist gained `repo` family + 11 acceptance + 5 contractor-flip cases). All 4 cutover steps now CLOSED (cutover-1 → cutover-2 → cutover-3 → cutover-4). **51 PRs merged this session. Frozen-layer additively touched only; 16 invariants preserved; 5 frozen contracts BYTE-IDENTICAL throughout entire session.** Master plan §0.5.6 NEW-A/B/C/D rows flip to `closed`. | d9a7ddbaa7 | Operator live-verify in Telegram (4 NEW + cutover-4 runbooks documented in respective PR bodies). Master-plan deferred items remain: Slice K reminder, Slice H P5/P6, Bug A.2 streaming buffering, Bug F persistent worker push, concurrent broker (PR-MT), bundle-as-contract, IntentContractor freshness/recency. |
| 2026-05-07 | **PR-MT concurrent broker SLICE COMPLETE** (PRs #294 (sub-plan landing) / #295 (Phase 1 audit) / #296 (Phase 2 types) / #297 (Phase 3 pure helpers) / #298 (Phase 4 broker runtime) / #299 (Phase 5 dispatch helper + agent-runner-execution wiring) / #300 (Phase 6 backpressure envelope + bootstrap binder) / this PR (Phase 7 acceptance + runbook + closure) — sub-plan landing + 7 phases: Phase 1 audit (`extensions/AUDIT-pr-mt-concurrent-broker.md` — 2 production callers + 2 internal-hook callers of `runTurnDecision` mapped; `FOLLOWUP_QUEUES` + implicit single-async-stack identified as 2 serial points; **zero** Mutex/Semaphore primitives in `src/`; identity available at `agent-runner-execution.ts` chokepoint via `params.sessionKey`; 17-piece gap list); Phase 2 types (NEW `src/platform/broker/broker-types.ts` — `BrokerQueueKey` brand (`<identityId>::<channelKey>`) + `BrokerEntry` envelope + `BrokerCapacityConfig` with Phase-2 defaults `maxQueueDepthPerKey=8` / `maxConcurrentKeys=32` / `queueWaitTimeoutMs=120_000` / `fairnessMode=round_robin` + Zod `.strict()` schema + closed `BROKER_OVERFLOW_REASONS`); Phase 3 pure helpers (NEW `src/platform/broker/decide-queue-placement.ts` — `decideQueuePlacement` admit/reject envelope (never throws #15; never mutates input; clock-injected via `nowMs`) + `decideNextDispatch` round-robin cursor (`keyOrder` + `lastServedKey`); ~9 fail-first cases); Phase 4 broker runtime (NEW `src/platform/broker/concurrent-turn-broker.ts` — `createConcurrentTurnBroker` factory; per-key FIFO `Map<BrokerQueueKey, BrokerEntry[]>` + `inFlight` Set + `keyOrder` cursor; `tryDispatch` loop drains keys per Phase 3 helper; structured `[broker] enqueued|dispatch|complete|rejected|shutdown` telemetry; `runTurn` exceptions swallowed (logged as `runTurn_threw`, not propagated); `shutdown()` drains in-flight; ~10 fail-first cases incl. single-key serial FIFO + two-key concurrency + round-robin fairness across 3 keys + clock-injectable); Phase 5 dispatch helper + wiring (NEW `src/auto-reply/reply/dispatch-turn-via-broker.ts` — composes `BrokerQueueKey` from `(identityId, JSON.stringify([channel,to,accountId,threadId]))`; `broker===undefined` falls through to direct `runTurn` invocation byte-identical to pre-broker baseline. Wired at `src/auto-reply/reply/agent-runner-execution.ts:186-246`. Plugin.ts callers OUT OF SCOPE per audit §1.2); Phase 6 backpressure + bootstrap (NEW `src/auto-reply/reply/format-broker-overflow-reply.ts` Russian-locale closed-set mapping + `deriveBrokerRetryAfterMs` deterministic via `getQueueDepth() * MS_PER_QUEUED_TURN_ESTIMATE` (no clock/randomness); NEW `src/server/concurrent-turn-broker-bootstrap.ts` process-scoped binder mirroring `delivery-receipt-registry` precedent — idempotent (`{kind:'bound'\|'alreadyBound'}`); wired into `src/gateway/server-startup.ts:31`. **Broker NOW LIVE in production**); Phase 7 acceptance + runbook + closure (NEW `src/platform/broker/__tests__/concurrent-turn-broker.acceptance.test.ts` — 9 acceptance cases over 8 describe blocks: 3 identity-distinct turns dispatch concurrently with overlapping wallclock < 250ms vs 300ms serial baseline + same-(identity, channel) FIFO preserved + round-robin fairness across 3 identities × 5 turns + backpressure overflow envelope with deterministic Russian-locale reply translation + reverse broker-disabled byte-identical baseline + shutdown drains in-flight emitting `[broker] shutdown drained=<N>` + telemetry log surface coverage + reverse-defense closed-set envelope assertion; NEW `extensions/RUNBOOK-pr-mt-concurrent-broker.md` operator runbook — pre-conditions + 3-identity 1s-window submission protocol + same-identity FIFO reverse + backpressure synthetic check + telemetry capture template + failure triage + rollback procedure; sub-plan frontmatter flipped `status: completed` + 7 todos `completed`; §6 Handoff Log entry written; master plan §0 row appended; master §0 references to PR-MT-deferred flip to CLOSED). **8 PRs merged this slice; frozen-layer source NEVER touched (`src/platform/commitment/**` BYTE-IDENTICAL throughout); 16 invariants preserved; 5 frozen contracts BYTE-IDENTICAL throughout (sha256 of `src/platform/decision/contracts.ts` = `57fc96305711690f5d75d99d63c673ee6389f624fdf7a6c750aad0dc02e624db`).** Closes operator-impact symptom: concurrent operator turns from different `(identityId, channelKey)` pairs no longer serialize on a single async stack; round-robin fairness prevents a hot identity from starving the rest. Pickup queue remaining: Slice H P5/P6 live-verify (operator), Bug A.2 streaming buffering, bundle-as-contract live-verify (operator). | &lt;final-sha&gt; | Operator live-verify in Telegram per `extensions/RUNBOOK-pr-mt-concurrent-broker.md` (3 identity-distinct turns within 1s window + same-identity FIFO reverse + optional backpressure synthetic). |
| 2026-05-07 | **Bundle-as-contract enforcement SLICE COMPLETE** (PRs #286 (sub-plan landing) / #287 (Phase 1 audit) / #288 (Phase 2 types) / #289 (Phase 3 pure filter) / #290 (Phase 4 attempt.ts wiring) / #291 (Phase 5 reverse-defense integration) / #292 (Phase 6 telemetry + acceptance) / this PR (Phase 7 runbook + closure) — sub-plan landing + 7 phases: Phase 1 audit (`extensions/AUDIT-bundle-as-contract.md` — schema-construction site at `attempt.ts:2004` (line drifted from sub-plan-cited `:1915` after PR-#145 / Search-Composer landings); 9 closed `ResolutionToolBundle` enum values + 6 advisory consumers + reverse-defense gap documented; structural source for `bundles` reachable via `params.platformExecutionContext: RecipeRuntimePlan`); Phase 2 types (NEW `src/agents/bundle-schema-filter.ts` — `BundleId` re-export of `ResolutionToolBundle` (single source of truth, NO new enum) + `BundleSchemaFilterConfig` + `DEFAULT_BUNDLE_ALLOWED_TOOLS` covering all 9 enum values via `Object.isFrozen` + closed-shape reverse-test + Zod `BundleSchemaFilterConfigSchema.strict()` + `resolveBundleAllowedTools` pure helper; closed allowlist mapping `respond_only`→`Set()`, `public_web_lookup`→`{web_search,web_fetch}`, `artifact_authoring`→`{pdf,docx,image_generate,apply_patch}`, etc.); Phase 3 pure filter (`filterToolSchemaByBundle` two-layer — bundle-allowlist + reverse-defense; never throws (#15); pure non-mutating; idempotent; ~9 fail-first cases incl. unknown tool name kept conservatively, unknown bundle id silently inert); Phase 4 wiring at `attempt.ts:2074-2087` (`applyBundleSchemaFilterAtAttempt` adapter applied AFTER `applyModelProviderToolPolicy` and BEFORE `disableWebSearchTool` / `sanitizeToolsForGoogle` post-processing; reads `params.platformExecutionContext.resolutionContract.toolBundles`; empty/undefined → byte-identical pass-through via `missingBundlePolicy: 'allow_all'` regression-guard; ~6 fail-first cases incl. 355ae135 shape replay producing empty schema, commands-system-prompt path UNCHANGED); Phase 5 reverse-defense integration (adversarial post-`applyModelProviderToolPolicy` `web_search` re-injection caught when `nativeWebSearchTool=false` AND no `public_web_lookup` bundle; `applyModelProviderToolPolicy` UNCHANGED — explicit non-goal documented; ~3 cases proving bundle-authority-wins for `public_web_lookup` + non-native model AND no double-fire on native-search models); Phase 6 telemetry + acceptance (`[bundle-filter] turnId=<id> bundles=[<...>] removed_tools=[<name>:<reason>,...] kept_tools=[<...>]` log line at filter exit; `info` when removals > 0, `debug` otherwise; defensive try/catch (#15) — logger throws cannot propagate into filter result; `bundle-schema-filter.acceptance.test.ts` 5 cases incl. **355ae135 reproduction** with empty tool schema asserted + `web_search:not_in_bundle_allowlist` log line asserted, **reverse absence-of-fix mode** proving regression direction, `public_web_lookup` happy path, composer combo, heartbeat-class regression guard `bundles=[]`+`allow_all`); Phase 7 runbook + closure (NEW `extensions/RUNBOOK-bundle-as-contract.md` — operator restart + 3 live turns covering `respond_only` («hi, how are you»), `public_web_lookup` («какие новости в мире»), `artifact_authoring` (PDF combo); expected `[bundle-filter]` telemetry per turn including bundle-authority-wins and reverse-defense cases; rollback procedure documented; sub-plan flipped `status: completed` + 7 todos `completed`; §6 Handoff Log entry written; master plan §0 row appended; master §0.5.1 row "Tool exposure согласована с model capability и bundle-контрактом" closure cell flipped to **CLOSED**; cross-links added to `commitment_kernel_search_composer_pipeline.plan.md` §8 row 1 + `orchestrator_web_search_capability_routing.plan.md` §8 row 1). **8 PRs merged this slice; frozen-layer source NEVER touched (`src/platform/commitment/**` BYTE-IDENTICAL throughout); 16 invariants preserved; 5 frozen contracts BYTE-IDENTICAL throughout (concat sha256 `75f926ef25f057f006ae9f48cd0eb624c392feea3206f6990e013df2cf23d25e` against predecessor `8384eb8e98`).** New filter file lives in `src/agents/` (orchestration / adapter layer per invariant #8); `BundleId` is RE-EXPORT of `ResolutionToolBundle` (consumer direction, not producer-inversion). Closes the 355ae135 turn-class symptom (autonomous DDG `web_search` → bot-detection → `Provider finish_reason: error` for classifier-mis-emit `bundles=[respond_only]` to non-native-search model). Pickup queue remaining: Slice H P5/P6 live-verify, Bug A.2 streaming buffering, concurrent broker (PR-MT, signoff). | &lt;final-sha&gt; | Operator live-verify in Telegram per `extensions/RUNBOOK-bundle-as-contract.md` (3 turns: `respond_only` / `public_web_lookup` / `artifact_authoring`). |
...
| Tool exposure согласована с model capability и bundle-контрактом | **CLOSED** by `commitment_kernel_bundle_as_contract.plan.md` SLICE COMPLETE 2026-05-07 (PRs #286–#292 + Phase 7 closure PR). LLM tool schema теперь фильтруется при schema-construction в `attempt.ts:2074-2087` через `applyBundleSchemaFilterAtAttempt(...)` — закрытый `DEFAULT_BUNDLE_ALLOWED_TOOLS` map (`respond_only`→∅; `public_web_lookup`→{web_search,web_fetch}; etc.) применяется ПОСЛЕ `applyModelProviderToolPolicy` и ДО `disableWebSearchTool`. Reverse-defense layer симметрично закрывает gap «не давать DDG-tool моделям без native search». Telemetry: `[bundle-filter] turnId=<...> bundles=[<...>] removed_tools=[...] kept_tools=[...]`. Empty `toolBundles` → byte-identical pass-through (`missingBundlePolicy: 'allow_all'`); 16 invariants preserved; 5 frozen contracts BYTE-IDENTICAL. Symptom turn-class `355ae135` (`Provider finish_reason: error` после autonomous DDG-search) — closed at the schema layer. | `src/agents/pi-embedded-runner/run/attempt.ts:2074-2087`, `src/agents/pi-embedded-runner/run/bundle-filter-wiring.ts`, `src/agents/bundle-schema-filter.ts`, `src/agents/bundle-schema-filter-apply.ts`, `extensions/RUNBOOK-bundle-as-contract.md`, `extensions/AUDIT-bundle-as-contract.md`. Sub-plan: `commitment_kernel_bundle_as_contract.plan.md` (`status: completed`). |
...
### 0.5.6. Live-test runtime gaps (2026-05-06) — orthogonal to v1 commitment-kernel slices

Live test on dev `735f65c019` after closure of Cutover-3 + PolicyGate Full + Search-Composer 4c surfaced 4 orchestration-layer bugs that block the end-to-end Telegram demo. These are NOT regressions of slices E / F / I / Cutover-3 / PolicyGate Full — those slice acceptance fixtures are green and frozen contracts are byte-identical. The bugs are pre-existing routing/streaming/session-isolation gaps that became visible only when v1 commitment-kernel slices stopped masking them. Each requires its own architectural sub-plan; no costyl/per-provider fixes per Vladimir's directive.

| ID    | Symptom (live evidence — gateway log `C:\tmp\openclaw\openclaw-2026-05-06.log`) | Root cause (architectural) | Future sub-plan (TBD) |
|-------|--------------------------------------------------------------------------------|----------------------------|------------------------|
| NEW-A | model-fallback layer orders `hydra/claude-opus-4.6` BEFORE `hydra/gpt-5.4` for some turns (lines L164/L750/L2865 — `route candidates ordered: hydra/claude-opus-4.6 -> hydra/gpt-5.4 -> ...`). Image-bearing turns routed to Opus silently lose attachment because Hydra's Opus declares `input_modalities=["text"]`. User-visible: `(JPG attachment) сделай PDF` returns text-only answer that ignores image. | Routing layer is provider-name-aware but NOT modality-aware. Architectural fix: candidate-ordering reads each model's `input_modalities` from the registry and filters by turn's modality requirements (the turn already carries structural data via Cutover-3 P6 `inboundMediaResolver`). Provider-agnostic — works identically when Opus/Sonnet/local-models change. | `commitment_kernel_modality_aware_routing.plan.md` |
| NEW-B | `/new` followed by «что я запомнил про X?» produces TWO assistant replies: (1) hallucinated answer parroting prior user message back, (2) actual memory-recall result (empty). Live screenshot 18:25 → 18:26 + 18:27. Memory store IS correctly empty after `/new`; chat-history is NOT. | `/new` clears memory-recall scope but does NOT clear chat-history kv. Model still sees prior user turn as context and hallucinates. Architectural fix: `/new` invokes a unified `resetTurnSession()` that atomically clears memory + chat-history kv + any other session-derived state. Single source of truth for session boundary. | `commitment_kernel_unified_session_reset.plan.md` |
| NEW-C | Two assistant messages emitted for one user prompt (Single_final_user_facing_message_per_user_turn invariant violation). Lane wait exceeded 79.5s on `session:agent:dev:main` (L2626 `lane wait exceeded: lane=session:agent:dev:main waitedMs=79565`). | Invariant exists at contract level but runtime has no per-turn coalescing buffer to merge intermediate messages before commit. Architectural fix: `OutboundCoalescer` layer that buffers per-`(turnId, channel)` outbound messages and emits one final user-facing message at turn end. Ack/preamble/intermediate/final all consolidate. | `commitment_kernel_outbound_coalescer.plan.md` |
| NEW-D | `[assistant-reply] runId=4407441e lang=en cyr=0 head="ALERT: Vladimir is waiting f..."` (L2392) — internal alert/meta string leaked outbound as user-facing message. `cyr=0` (zero Cyrillic) on a channel with `locale=ru`. | Slice I sanitizer (`outbound-sanitizer.ts`) covers `english_meta_*` patterns but is NOT channel-locale-aware. Architectural fix: outbound sanitizer reads channel locale from the existing channel-policy and blocks non-locale-matching system-meta strings (additive policy on the existing surface — slice I P5 + P6 precedent). | `commitment_kernel_locale_aware_sanitizer.plan.md` |

**Status (2026-05-07)**: All 4 NEW-A/B/C/D bugs CLOSED (PRs #214–#225/#227–#237/#226–#233; 24 PRs across the four slices). Master plan §0 line 134 `closed` flip confirmed. Code shipped per spec + acceptance fixtures green; live Telegram operator verify is the remaining gate.

**Master plan completion status (2026-05-07)**: v1 commitment-kernel slices CLOSED (E / F / I / Cutover-3 / PolicyGate Full / Search-Composer pipeline). cutover-4 (`repo_operation.completed`) CLOSED. NEW-A/B/C/D CLOSED. Slice K (reminder) CLOSED. Slice H P5 CLOSED (P6 live-verify = operator). Bug A.2 (streaming buffering) STAND DOWN per audit. IntentContractor freshness/recency CLOSED (PRs #266–#271). Bug F (persistent worker push) CLOSED. **Concurrent broker (PR-MT) CLOSED** (PRs #294–#300 + Phase 7 closure PR; broker LIVE in production via `src/server/concurrent-turn-broker-bootstrap.ts`; per-`(identityId, channelKey)` FIFO + cross-key concurrency + round-robin fairness + structured overflow envelope). Bundle-as-contract enforcement CLOSED. **Open frontier**: operator live-verify of all completed slices (PR-MT runbook at `extensions/RUNBOOK-pr-mt-concurrent-broker.md`; bundle-as-contract runbook at `extensions/RUNBOOK-bundle-as-contract.md`; Slice H P6 / Slice K live-verify). Master plan is **NEAR COMPLETE** — only operator-gated items remain.
