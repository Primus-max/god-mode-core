...
describe("ui i18n locale registry", () => {
  it("lists supported locales", () => {
    expect(SUPPORTED_LOCALES).toEqual(["en", "zh-CN", "zh-TW", "pt-BR", "de", "es", "ru"]);
  ...
  it("resolves browser locale fallbacks", () => {
    ...
    expect(resolveNavigatorLocale("ru-RU")).toBe("ru");
  ...
  it("loads lazy locale translations from the registry", async () => {
    ...
    const ru = await loadLazyLocaleTranslation("ru");
    ...
    expect(getNestedTranslation(ru, "common", "health")).toBe("Состояние");
    expect(getNestedTranslation(ru, "tabs", "nodes")).toBe("Устройства и узлы");
