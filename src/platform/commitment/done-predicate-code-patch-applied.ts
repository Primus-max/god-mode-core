...
/**
 * Verifies that the runtime adapter (Phase 5) recorded a `code_patch`
 * artifact whose `artifactId` was emitted via the commitment's
 * `expectedDelta.artifacts.added` list. Mirrors `pdfCreatedPredicate`
 * (slice cutover-3 Phase 4) — see that file for the closed missing-key
 * set rationale.
 *
 ...
 */
export const codePatchAppliedPredicate: DonePredicate = (ctx) => {
  ...
  for (const artifactId of expectedAdded) {
    ...
    if (!record || record.kind !== "code_patch") {
