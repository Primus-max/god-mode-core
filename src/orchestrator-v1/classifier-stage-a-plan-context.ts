...
ОБЩИЙ ПРИНЦИП ЗАМЕНЫ ИНСТРУМЕНТА:
Если пользователь ОТРИЦАЕТ один из инструментов плана ("не <tool>", "а не <tool>", "не надо <action>") и предлагает ДРУГОЕ действие над тем же объектом — это EDIT_PLAN: убери отрицаемый инструмент из tool_names, поставь подходящий инструмент. Остальные инструменты плана сохрани в том же порядке. Отрицание + альтернатива = семантическая замена, а не отказ.

ТИПИЧНЫЕ ЗАМЕНЫ (учись по смыслу, не по словам):
- image_generate → web_search: пользователь говорит "найти картинки", "поискать изображения", "не генерируй, найди в инете". Семантика: ищем существующее, а не создаём.
- image_generate → web_fetch: пользователь дал URL и просит скачать ("скачай вот эту картинку <url>"). Семантика: конкретный адрес → загрузка.
- pdf → write (в .docx/.md/.txt): пользователь говорит "не PDF, а Word", "шаблоны word, а не pdf", "сделай в .docx". Семантика: смена формата документа — pdf-инструмент убирается, форматирование переходит к write.
- write → edit: пользователь говорит "не создавай новый файл, поправь существующий", "отредактируй то что есть". Семантика: модификация vs создание.
- web_search → web_fetch: пользователь после неуспешного поиска даёт прямой URL ("вот ссылка <url>, оттуда возьми").
- web_fetch → web_search: пользователь не дал URL и говорит "поищи в инете" / "найди где скачать".

Эти примеры НЕ исчерпывающий список — применяй тот же принцип к любой паре инструментов. Если пользователь упоминает несколько правок в одном сообщении (например "не pdf, шаблоны word, и не генерируй а найди") — учти все: убери pdf-инструмент, замени image_generate на web_search, остальное оставь.
