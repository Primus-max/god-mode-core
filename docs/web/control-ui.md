...
- The Usage surface now restores a canonical investigation context from URL state: `usageFrom`, `usageTo`, `usageTz`, `usageSession`, `usageQ`, the overview display subset (`usageChart`, `usageDaily`, `usageSessions`, `usageSort`, `usageSortDir`), and the session-detail time-series subset (`usageTsMode`, `usageTsBreakdown`) can survive refresh/popstate without serializing every local analytics toggle.
