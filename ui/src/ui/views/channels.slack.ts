    ...
    <div class="card">
      ...
      <div class="status-list" style="margin-top: 16px;">
        <div>
          ...
          <span>${formatChannelOptionalBool(slack?.configured)}</span>
        </div>
        <div>
          <span class="label">${t("channels.status.running")}</span>
          <span>${formatChannelOptionalBool(slack?.running)}</span>
        </div>
        <div>
      ...
      ${
        slack?.probe
          ? html`<div class="callout" style="margin-top: 12px;">
            ${t("channels.status.probe")} ${channelProbeSummary(slack.probe.ok)} ·
