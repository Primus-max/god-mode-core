    ...
    <div class="card">
      ...
      <div class="status-list" style="margin-top: 16px;">
        <div>
          ...
          <span>${formatChannelOptionalBool(imessage?.configured)}</span>
        </div>
        <div>
          <span class="label">${t("channels.status.running")}</span>
          <span>${formatChannelOptionalBool(imessage?.running)}</span>
        </div>
        <div>
      ...
      ${
        imessage?.probe
          ? html`<div class="callout" style="margin-top: 12px;">
            ${t("channels.status.probe")} ${channelProbeSummary(imessage.probe.ok)} ·
