...
import { channelProbeSummary, formatChannelBool } from "./channels.shared.ts";
    ...
    <div class="card">
      ...
      <div class="status-list" style="margin-top: 16px;">
        <div>
          ...
          <span>${googleChat ? formatChannelBool(googleChat.configured) : t("common.na")}</span>
        </div>
        <div>
          <span class="label">${t("channels.status.running")}</span>
          <span>${googleChat ? formatChannelBool(googleChat.running) : t("common.na")}</span>
        </div>
        <div>
      ...
      ${
        googleChat?.probe
          ? html`<div class="callout" style="margin-top: 12px;">
            ${t("channels.status.probe")} ${channelProbeSummary(googleChat.probe.ok)} ·
