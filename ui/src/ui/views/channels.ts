  ...
  return html`
    ...
    <section class="card" style="margin-top: 18px;">
      <div class="row" style="justify-content: space-between;">
        ...
        <div class="muted">${
          props.lastSuccessAt
            ? t("channels.lastUpdated", { time: formatRelativeTimestamp(props.lastSuccessAt) })
            : t("common.na")
        }</div>
      </div>
      ${
          ...
          : html`
            <div class="status-list" style="margin-top: 16px;">
              <div>
                ...
                <span>${formatChannelOptionalBool(configured)}</span>
              </div>
              <div>
                <span class="label">${t("channels.status.running")}</span>
                <span>${formatChannelOptionalBool(running)}</span>
              </div>
              <div>
                <span class="label">${t("channels.status.connected")}</span>
                <span>${formatChannelOptionalBool(connected)}</span>
              </div>
            </div>
          `
      }
...
function renderGenericAccount(account: ChannelAccountSnapshot) {
  const runningStatus = formatChannelRunDisplay(deriveChannelRunningState(account));
  const connectedStatus = formatChannelConnectedDisplay(deriveChannelConnectedState(account));
