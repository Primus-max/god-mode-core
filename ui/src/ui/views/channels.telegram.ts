        ...
        hasMultipleAccounts
          ...
          : html`
            <div class="status-list" style="margin-top: 16px;">
              <div>
                <span class="label">${t("channels.status.configured")}</span>
                <span>${formatChannelOptionalBool(telegram?.configured)}</span>
              </div>
              <div>
                <span class="label">${t("channels.status.running")}</span>
                <span>${formatChannelOptionalBool(telegram?.running)}</span>
              </div>
              <div>
      ...
      ${
        telegram?.probe
          ? html`<div class="callout" style="margin-top: 12px;">
            ${t("channels.status.probe")} ${channelProbeSummary(telegram.probe.ok)} ·
