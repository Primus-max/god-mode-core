...
export function renderDiscordCard(params: {
  ...
  return html`
    <div class="card">
      ...
      <div class="card-sub">${t("channels.cards.discordSubtitle")}</div>
      ${accountCountLabel}

      <div class="status-list" style="margin-top: 16px;">
        <div>
          <span class="label">${t("channels.status.configured")}</span>
          <span>${formatChannelOptionalBool(discord?.configured)}</span>
        </div>
        <div>
          <span class="label">${t("channels.status.running")}</span>
          <span>${formatChannelOptionalBool(discord?.running)}</span>
        </div>
        <div>
      ...
      ${
        discord?.probe
          ? html`<div class="callout" style="margin-top: 12px;">
            ${t("channels.status.probe")} ${channelProbeSummary(discord.probe.ok)} ·
