    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        ...
        <button class="btn" ?disabled=${props.loading} @click=${props.onRefresh}>
          ${props.loading ? t("nodes.loading") : t("common.refresh")}
        </button>
      </div>
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        ...
        <button class="btn" ?disabled=${props.devicesLoading} @click=${props.onDevicesRefresh}>
          ${props.devicesLoading ? t("nodes.loading") : t("common.refresh")}
        </button>
      </div>
      ${
        props.devicesError
...
function renderPendingDevice(req: PendingDevice, props: NodesProps) {
  ...
  const repair = req.isRepair ? t("nodes.repairSuffix") : "";
...
function renderTokenRow(deviceId: string, token: DeviceTokenSummary, props: NodesProps) {
  const status = token.revokedAtMs ? t("nodes.tokenStatusRevoked") : t("nodes.tokenStatusActive");
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
          <div class="card-title">${t("nodes.execBindingTitle")}</div>
          <div class="card-sub">${t("nodes.execBindingSubtitle")}</div>
        </div>
        <button
          class="btn"
          ...
          @click=${state.onSave}
        >
          ${state.configSaving ? t("nodes.saving") : t("nodes.save")}
        </button>
      </div>
        ...
        !state.ready
          ...
          : html`
            <div class="list" style="margin-top: 16px;">
              <div class="list-item">
                <div class="list-main">
                  <div class="list-title">${t("nodes.defaultBindingTitle")}</div>
                ...
                <div class="list-meta">
                  ...
                  ${
                    !supportsBinding
                      ? html`
                          <div class="muted">${t("nodes.noBindableNodes")}</div>
                        `
                      : nothing
                  }
                </div>
        ...
        <div class="list-sub">
          ...
          ${
            bindingValue === "__default__"
              ...
              : t("nodes.override", { value: String(agent.binding) })
          }
        </div>
      </div>
      <div class="list-meta">
        <label class="field">
          <span>${t("nodes.bindingLabel")}</span>
          <select
            ?disabled=${state.disabled || !supportsBinding}
            @change=${(event: Event) => {
              const target = event.target as HTMLSelectElement;
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="chip-row" style="margin-top: 6px;">
          <span class="chip">${paired ? t("nodes.chips.paired") : t("nodes.chips.unpaired")}</span>
          <span class="chip ${connected ? "chip-ok" : "chip-warn"}">
            ${connected ? t("nodes.chips.connected") : t("nodes.chips.offline")}
          </span>
