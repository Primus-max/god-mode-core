...
export function renderChannelConfigForm(props: ChannelConfigFormProps) {
  ...
  if (!normalized) {
    return html`
      <div class="callout danger">${t("channels.configSection.schemaUnavailable")}</div>
    `;
  }
  const node = resolveSchemaNode(normalized, ["channels", props.channelId]);
  if (!node) {
    return html`
      <div class="callout danger">${t("channels.configSection.channelSchemaUnavailable")}</div>
    `;
  }
    ...
    <div style="margin-top: 16px;">
      ${
        props.configSchemaLoading
          ? html`
              <div class="muted">${t("channels.configSection.loadingSchema")}</div>
            `
      ...
      <div class="row" style="margin-top: 12px;">
        ...
        >
          ${props.configSaving ? t("channels.configSection.saving") : t("channels.configSection.save")}
        </button>
        <button
          class="btn"
          ?disabled=${disabled}
        ...
        >
          ${t("channels.configSection.reload")}
        </button>
      </div>
    </div>
  `;
